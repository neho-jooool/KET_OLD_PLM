 ...
 *
 * <p>
 * Use the <code>newOEMPlan</code> static factory method(s), not the <code>OEMPlan</code>
...
@GenAsPersistable(
   ...
   properties={
   @GeneratedProperty(name="oemEndDate", type=Timestamp.class),
   @GeneratedProperty(name="customer", type=String.class),
   @GeneratedProperty(name="viewType", type=int.class)
   },
   foreignKeys={
   @GeneratedForeignKey(
      foreignKeyRole=@ForeignKeyRole(name="oemType", type=e3ps.project.outputtype.OEMProjectType.class),
      myRole=@MyRole(name="theOEMPlan", cardinality=Cardinality.ONE)),
   @GeneratedForeignKey(myRoleIsRoleA=false,
      foreignKeyRole=@ForeignKeyRole(name="modelPlan", type=e3ps.project.outputtype.ModelPlan.class),
      myRole=@MyRole(name="theOEMPlan", cardinality=Cardinality.ONE))
   })
public class OEMPlan extends _OEMPlan {
    ...
    **/
   public static OEMPlan newOEMPlan()
            throws WTException {

      OEMPlan instance = new OEMPlan();
      instance.initialize();
      return instance;
   }
