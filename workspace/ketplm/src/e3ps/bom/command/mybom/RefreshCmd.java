...
public class RefreshCmd extends AbstractAIFCommand
{
	...
	public RefreshCmd(JFrame frame, AbstractAIFUIApplication app)
    {
        this.app = app;
        parent = null;
        Desktop = frame;
    ...
    protected void executeCommand() throws Exception
    {
    	try
    	{
			...
			if(BOMBasicInfoPool.getPublicModelName().equals(""))
			{
				pnl.setCursor(Cursor.getPredefinedCursor(0));
				MessageBox m = new MessageBox(app.getDesktop(), messageRegistry.getString("openBOMWorkspace"), "Error", MessageBox.ERROR);
				m.setModal(true);
				m.setVisible(true);
				return;
			}
			...
			String bomOid = BOMBasicInfoPool.getPublicBOMOid() == null ? "" : BOMBasicInfoPool.getPublicBOMOid().trim();

			if (bomGubunFlag) {		// 신규일 때
