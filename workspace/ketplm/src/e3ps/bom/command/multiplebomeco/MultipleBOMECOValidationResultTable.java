...
public  class MultipleBOMECOValidationResultTable extends JTable
{
    ...
    public MultipleBOMECOValidationResultTable(JFrame frame, AbstractAIFUIApplication app)
    {
        super();
        this.frame = frame;
        this.app = app;
    ...
    public MultipleBOMECOValidationResultTable()
    {
    }

    public void updateTableView()
    {
        getColumn("No").setMaxWidth(40);
        getColumn(KETMessageService.service.getString("e3ps.bom.bom_editor_message", "00098")/*구분*/).setMaxWidth(60);
        getColumn(KETMessageService.service.getString("e3ps.bom.bom_editor_message", "00152")/*모부품번호*/).setMinWidth(135);
        getColumn(KETMessageService.service.getString("e3ps.bom.bom_editor_message", "00184")/*부품번호*/).setMinWidth(135);
        getColumn(KETMessageService.service.getString("e3ps.bom.bom_editor_message", "00125")/*내용*/).setMinWidth(302);

        for(int inx=0;inx<getColumnCount();inx++)
        {
            setDefaultRenderer(getColumnClass(inx),new MultipleBOMECOValidationTableCellRenderer());
        }
    }
