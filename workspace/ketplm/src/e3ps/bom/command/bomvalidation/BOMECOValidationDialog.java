...
public class BOMECOValidationDialog extends JDialog {
    ...
    private JLabel titleLbl, totalCntLbl, warningLbl, errorLbl, validationLbl1, validationLbl2, validationLbl3, validationLbl4,
	    validationLbl5, validationLbl6, validationLbl7, validationLbl8, validationLbl9, validationLbl10, validationLbl11,
	    validationLbl12, validationLbl13; // , validationLbl14, validationLbl15;
    private JCheckBox validationChk1, validationChk2, validationChk3, validationChk4, validationChk5, validationChk6, validationChk7,
	    validationChk8, validationChk9, validationChk10, validationChk11, validationChk12, validationChk13; // , validationChk14,
														// validationChk15;
    ...
    // private String ecoHeaderNumber = Utility.checkNVL(BOMBasicInfoPool.getBomEcoNumber());
    ...
    public BOMECOValidationDialog(JFrame frame, AbstractAIFUIApplication app) {
	...
	try {
	    ...
	    Kogger.debug(getClass(), "========2 rootNode : " + rootNode);
	    Kogger.debug(getClass(), "========2 ChildCount : " + rootNode.getChildCount());
	    Kogger.debug(getClass(), "========2 getChildren : " + rootNode.getChildren().length);
	    // ///////////////////////////////////////////////////////////////////////////////////
	    ...
	    okBtn = new JButton(KETMessageService.service.getString("e3ps.bom.bom_editor_message", "00259")/* 실행 */,
		    appReg.getImageIcon("bomvalidationIcon"));
	    okBtn.setFont(FontList.defaultFont);
	    okBtn.setMargin(new Insets(0, 5, 0, 5));
	    cancelBtn = new JButton(KETMessageService.service.getString("e3ps.bom.bom_editor_message", "00341")/* 취소 */,
		    appReg.getImageIcon("closeIcon"));
	    ...
	    validationLbl1 = new JLabel(KETMessageService.service.getString("e3ps.bom.bom_editor_message", "00004", 1)/*
							                                                               * {0} 단계 : BOM 구성에
							                                                               * 필요한 부품이 PLM에 채번되어
							                                                               * 있는지 체크
							                                                               */);
	    validationLbl1.setFont(FontList.defaultFont);
	    validationLbl2 = new JLabel(KETMessageService.service.getString("e3ps.bom.bom_editor_message", "00012", 2)/*
							                                                               * {0} 단계 : 부품코드와
							                                                               * 대체부품코드가 동일한지 체크
							                                                               */);
	    validationLbl2.setFont(FontList.defaultFont);
	    validationLbl3 = new JLabel(KETMessageService.service.getString("e3ps.bom.bom_editor_message", "00017", 3)/*
							                                                               * {0} 단계 : 필수 속성값(제품
							                                                               * : 수량, 단위 , 금형 : 수량,
							                                                               * 재질, 경도(From),
							                                                               * 경도(To)) 이 존재하는지 체크
	    ...
	    validationLbl4 = new JLabel(KETMessageService.service.getString("e3ps.bom.bom_editor_message", "00006", 4)/*
							                                                               * {0} 단계 : 모부품의 동일한
							                                                               * 레벨에 같은 부품이 사용되었는지
							                                                               * 체크
							                                                               */);
	    validationLbl4.setFont(FontList.defaultFont);
	    validationLbl5 = new JLabel(KETMessageService.service.getString("e3ps.bom.bom_editor_message", "00007", 5)/*
							                                                               * {0} 단계 : 모부품이 하위
							                                                               * 자부품으로 재사용되었는지 체크
							                                                               */);
	    validationLbl5.setFont(FontList.defaultFont);
	    validationLbl6 = new JLabel(KETMessageService.service.getString("e3ps.bom.bom_editor_message", "00005", 6)/*
							                                                               * {0} 단계 : 동일한 모부품이
							                                                               * 서로 다른 구조의 자부품으로
							                                                               * 사용되었는지 체크
							                                                               */);
	    validationLbl6.setFont(FontList.defaultFont);
	    // validationLbl7 = new JLabel(" 7 단계 : 변경 이관하려는 BOM 구조가 이미 BOM마스터에 적용되었는지 체크");
	    // validationLbl7.setFont(FontList.defaultFont);
	    validationLbl8 = new JLabel(KETMessageService.service.getString("e3ps.bom.bom_editor_message", "00011", 7)/*
							                                                               * {0} 단계 : 부품중에서 ERP로
							                                                               * I/F 되지 않은 신규 부품 존재
							                                                               * 여부 체크
							                                                               */);
	    validationLbl8.setFont(FontList.defaultFont);
	    validationLbl9 = new JLabel(KETMessageService.service.getString("e3ps.bom.bom_editor_message", "00010", 8)/*
							                                                               * {0} 단계 : 부품이 체크인
							                                                               * 상태인지 체크
							                                                               */);
	    validationLbl9.setFont(FontList.defaultFont);
	    validationLbl10 = new JLabel(KETMessageService.service.getString("e3ps.bom.bom_editor_message", "00008", 9)/*
							                                                                * {0} 단계 : 부품에 연결된
							                                                                * 도면/문서가 체크인 상태인지 체크
							                                                                */);
	    validationLbl10.setFont(FontList.defaultFont);
	    validationLbl11 = new JLabel(KETMessageService.service.getString("e3ps.bom.bom_editor_message", "00009", 10)/*
								                                                         * {0} 단계 : 부품에 연결된
								                                                         * 비철/수지 원재료 코드 밑에
								                                                         * 올바른 스트랩 코드가 존재하는지
								                                                         * 체크
	    ...
	    validationLbl12 = new JLabel(KETMessageService.service.getString("e3ps.bom.bom_editor_message", "00015", 11)/*
								                                                         * {0} 단계 : 터미널
								                                                         * 반제품일때 자부품에
								                                                         * 후도금제품코드가 존재하는지 체크
								                                                         */);
	    validationLbl12.setFont(FontList.defaultFont);
	    validationLbl13 = new JLabel(KETMessageService.service.getString("e3ps.bom.bom_editor_message", "00013", 12)/*
								                                                         * {0} 단계 : 삭제된 부품이
								                                                         * 존재하는지 체크
								                                                         */);
	    ...
	    validationChk11.setBounds(new Rectangle(12, 222, 20, 23));
	    validationChk12.setBounds(new Rectangle(12, 246, 20, 23));
	    validationChk13.setBounds(new Rectangle(12, 270, 20, 23));
	    ...
	    validationLbl11.setBounds(new Rectangle(40, 222, 490, 23));
	    validationLbl12.setBounds(new Rectangle(40, 246, 490, 23));
	    validationLbl13.setBounds(new Rectangle(40, 270, 490, 23));
	...
	cancelBtn.addActionListener(new ActionListener() {
		...
		if (strValidationForEnd.equals("LAST")) {
			...
			MessageBox m = new MessageBox(desktop, messageRegistry.getString("valid1"), KETMessageService.service.getString(
			        "e3ps.bom.bom_editor_message", "00021")/* BOM 검증 오류 */, MessageBox.ERROR);
			m.setVisible(true);
			m.setModal(true);
			return;
		    } else {
	...
	try {
	    ...
	    loadWarningMessage(messageRegistry.getString("valid4"));
	}
    }
	...
	try {
	    ...
	    loadWarningMessage(messageRegistry.getString("valid5"));
	}
    }
	...
	try {
	    ...
	    Kogger.debug(getClass(), "@@@@ totalDataCount : " + totalDataCount);
		...
		// 7 Stage : 변경 이관하려는 BOM 구조가 이미 BOMMaster에 적용되었는지 체크
		// timer = new javax.swing.Timer(TIME_UNIT, new TimerListener());
		// timer.start();
		// validationChk6.setSelected(false);
		...
		if (warningCount == 0) {
		    ...
		    loadWarningMessage(fr,
			    KETMessageService.service.getString("e3ps.bom.bom_editor_message", "00022", errorCount, warningCount)/*
								                                                                  * BOM 검증
								                                                                  * 완료!\n에러:
								                                                                  * {0}개,
								                                                                  * 경고: {1}개
	...
	try {
		...
		for (int inx = 0; inx < subCompsItemCodeVec.size(); inx++) {
		    ...
		    substituteItemCodeStr = subAssyComponent.getSubstituteItemCodeStr() == null ? "" : subAssyComponent
			    .getSubstituteItemCodeStr().toString().trim();
		...
		if (!cmp.getItemCodeStr().trim().equalsIgnoreCase(modelName)) {
		    ...
		    if ((parentCmp.getItemCodeStr() == null ? "" : parentCmp.getItemCodeStr().trim()).equals(modelName)) // TODO 신규와 다름
															 // if((parentCmp.getNewFlagStr()
															 // == null? "" :
															 // parentCmp.getNewFlagStr()).equals("NEW"))
		    {
			if (itemVec.indexOf(cmp.getItemCodeStr().trim()) == -1) {
			    resultList.addElement(parentCmp.getItemCodeStr() + "|" + cmp.getItemCodeStr().trim() + "^" + cmp.getSeqInt()
				    + "#Comps");
			}
			...
			for (int inx = 0; inx < substituteVec.size(); inx++) {
				...
				resultList.addElement(parentCmp.getItemCodeStr() + "|" + substituteItemCodeStr + "^" + cmp.getSeqInt()
				        + "#SubComps");
			    }
			}
		    }
	...
	if (resultList.size() > 0) {
		...
		Vector v = new Vector();
		v.add(String.valueOf(resultList.elementAt(i).toString()
		        .substring(resultList.elementAt(i).toString().indexOf("^") + 1, resultList.elementAt(i).toString().indexOf("#"))
		        + " "));
		...
		v.add(" "
		        + resultList
		                .elementAt(i)
		                .toString()
		                .substring(resultList.elementAt(i).toString().indexOf("|") + 1,
		                        resultList.elementAt(i).toString().indexOf("^")));
		...
		if (String
		        .valueOf(
		                resultList
		                        ...
		                        .toString()
		                        .substring(resultList.elementAt(i).toString().indexOf("#") + 1,
		                                resultList.elementAt(i).toString().length())).equals("Comps")) {
	...
	try {
		...
		for (int i = 0; i < substituteItemVec.size(); i++) {
		    if ((parentCmp.getItemCodeStr() == null ? "" : parentCmp.getItemCodeStr().trim()).equals(modelName)) // TODO 신규와 다름
		    {
			if ((bomassy.toString().equals(substituteItemVec.elementAt(i)))
			        && (bomassy.getSortOrderStr().equals(seqVec.elementAt(i)))) {
			    Vector v = new Vector();
    ...
    // 3 Stage : 필수 속성값(Supply Type, Quantity, StartDate, Cost Rollup) 이 존재하는지 체크
	...
	while (enum0.hasMoreElements()) {
	    ...
	    if ((bomparent.getItemCodeStr() == null ? "" : bomparent.getItemCodeStr().trim()).equals(modelName)) // TODO 신규와 다름
	    {
		// 수량은 제품,금형 공용
	...
	while (enum0.hasMoreElements()) {
	    ...
	    if (bomassy.getLevelInt().intValue() == 1) // TODO 신규와 다름 if (bomassy.getNewFlagStr().equalsIgnoreCase("NEW") &&
						       // bomassy.getLevelInt().intValue() != 0)
	    {
		for (int inx = 0; inx < allListNode.getChildCount(); inx++) {
			...
			if ((assy1.getLevelInt().intValue() - assy2.getLevelInt().intValue() == 0)
			        && assy1.getItemCodeStr().equals(assy2.getItemCodeStr()) && (inx != jnx)) {
			    Vector v = new Vector();
			    v.add(assy1.getSeqInt().toString() + " ");
			    v.add(" " + KETMessageService.service.getString("e3ps.bom.bom_editor_message", "00266")/* 오류 */);
			    v.add(" " + assy1.getParentItemCodeStr());
			    v.add(" " + assy1.toString());
			    v.add(" " + KETMessageService.service.getString("e3ps.bom.bom_editor_message", "00140")/*
							                                                            * 동일한 assy안에 같은 레벨의 부품이
							                                                            * 중복되었습니다.
							                                                            */);
			    resultVector.add(v);
			    errorCount++;
	...
	while (enum0.hasMoreElements()) {
		...
		for (int inx = 0; inx < allListNode.getChildCount(); inx++) {
			...
			BOMAssyComponent assy1 = ((BOMTreeNode) allListNode.getChildAt(inx)).getBOMComponent();
			BOMAssyComponent assy2 = ((BOMTreeNode) allListNode.getChildAt(jnx)).getBOMComponent();
			if ((assy1.getLevelInt().intValue() - assy2.getLevelInt().intValue() == 1)
			        && assy1.getItemCodeStr().equals(assy2.getItemCodeStr())) {
			    Vector v = new Vector();
			    v.add(assy1.getSeqInt().toString() + " ");
			    v.add(" " + KETMessageService.service.getString("e3ps.bom.bom_editor_message", "00266")/* 오류 */);
			    v.add(" " + (assy1.getParentItemCodeStr() == null ? "" : assy1.getParentItemCodeStr().trim()));
			    v.add(" " + assy1.toString());
			    v.add(" " + KETMessageService.service.getString("e3ps.bom.bom_editor_message", "00141")/*
							                                                            * 동일한 assy에 모부품과 자부품이
							                                                            * 중복되었습니다.
							                                                            */);
			    resultVector.add(v);
	...
	while (enum0.hasMoreElements()) {
	    ...
	    if (bomassy.getLevelInt().intValue() == 1) // TODO 신규와 다름 if ((bomassy.getNewFlagStr()).equals("NEW"))
	    {
		itemCodeUpStr = bomassy.toString();

		// 구성중인 BOM 비교를 위해 vector에 넣는다.
	...
	try {
		...
		if (!cmp.getItemCodeStr().trim().equalsIgnoreCase(modelName)) {
		    BOMAssyComponent parentCmp = ((BOMTreeNode) selectedNode.getParent()).getBOMComponent();
		    if ((parentCmp.getItemCodeStr() == null ? "" : parentCmp.getItemCodeStr().trim()).equals(modelName))
	...
	try {
	    ...
	    Kogger.debug(getClass(), "coworker : " + coworkerVec.toString());
		...
		for (int i = 0; i < coworkerVec.size(); i++) {
		    if (modelName.equals(cmp.getItemCodeStr())) {
			if ((coworkerVec.elementAt(i) == null ? "" : coworkerVec.elementAt(i).toString().trim()
			        .substring(0, coworkerVec.elementAt(i).toString().trim().indexOf("|"))).equals(cmp.getItemCodeStr().trim())) {
		    ...
		    if (String.valueOf(
			    resultList
			            ...
			            .substring(resultList.elementAt(i).toString().indexOf("#") + 1,
			                    resultList.elementAt(i).toString().length())).equals("Comps")) {
			v.add(" "
			        + KETMessageService.service.getString("e3ps.bom.bom_editor_message", "00197", resultList.elementAt(i)
			                .toString().substring(0, resultList.elementAt(i).toString().indexOf("$")))/*
							                                                           * 부품코드가 {0}에 의해 체크아웃
							                                                           * 상태입니다.
							                                                           */);
		    } else {
			v.add(" "
			        + KETMessageService.service.getString("e3ps.bom.bom_editor_message", "00135", resultList.elementAt(i)
			                .toString().substring(0, resultList.elementAt(i).toString().indexOf("$")))/*
							                                                           * 대체부품코드가 {0}에 의해 체크아웃
							                                                           * 상태입니다.
							                                                           */);
	...
	while (enum0.hasMoreElements()) {
		...
		if (firstItem.equals("R10")) {
			...
			v.add(" " + KETMessageService.service.getString("e3ps.bom.bom_editor_message", "00207")/*
							                                                        * 비철 원재료코드 아래의 스트랩 코드가 기준과
							                                                        * 다릅니다.
							                                                        */);
			resultVector.add(v);
			warningCount++;
			...
			v.add(" " + bomcomponent.getParentItemCodeStr());
			v.add(" " + bomcomponent.toString());
			v.add(" " + KETMessageService.service.getString("e3ps.bom.bom_editor_message", "00246")/*
							                                                        * 수지 원재료코드 아래의 스트랩 코드가 기준과
							                                                        * 다릅니다.
							                                                        */);
			resultVector.add(v);
			warningCount++;
	    ...
	    /*
	     * else //head... {
	     * Kogger.debug(getClass(), "---------------------------------------->>>> else : "+BOMBasicInfoPool.getPublicModelName().trim());
	     * //nodeVec.add(BOMBasicInfoPool.getPublicModelName().trim()); }
	     */
	}
	...
	while (enum0.hasMoreElements()) {
		...
		if ((firstItem.equals("H71") || firstItem.equals("H72") || firstItem.equals("H73") || firstItem.equals("H74")
		        || firstItem.equals("H75") || firstItem.equals("H76") || firstItem.equals("H78") || firstItem.equals("H79"))
			...
			v.add(" " + bomcomponent.getParentItemCodeStr());
			v.add(" " + bomcomponent.toString());
			v.add(" " + KETMessageService.service.getString("e3ps.bom.bom_editor_message", "00344")/*
							                                                        * 터미널 반제품코드의 자부품이 후도금 제품코드가
							                                                        * 아닙니다.
							                                                        */);
			resultVector.add(v);
			warningCount++;
	...
	try {
		...
		BOMTreeNode allListNode = (BOMTreeNode) enum0.nextElement();
		BOMAssyComponent bomcomponent = (BOMAssyComponent) allListNode.getBOMComponent();
		Kogger.debug(getClass(), "@@@@ 1 bomcomponent : " + bomcomponent);
		if (allListNode.getParent() == null) {
		    continue;
		}
		...
		if ((bomparent.getItemCodeStr() == null ? "" : bomparent.getItemCodeStr().trim()).equals(modelName)) {
		    nodeVec.add(bomcomponent.getItemCodeStr() == null ? "" : bomcomponent.getItemCodeStr().trim());
		}
	    }
		...
		BOMTreeNode selectedNode = (BOMTreeNode) enum1.nextElement();
		BOMAssyComponent cmp = selectedNode.getBOMComponent();
		Kogger.debug(getClass(), "@@@@ 2 cmp " + cmp);
		if (selectedNode.getParent() == null) {
		    continue;
		}
		...
		if (indx > -1) {
			...
			resultList
			        .addElement(parentCmp.getItemCodeStr().trim() + "|" + cmp.getItemCodeStr().trim() + "^" + cmp.getSeqInt());
		    }
		}
	    }
	    ...
	    Kogger.error(getClass(), e);
	    loadWarningMessage("From 12 Stage error : " + e.toString());
	} finally {
	    if (resource != null)
		resource.freeConnection(appReg.getString("plm"), connection);
	}
	...
	errorLbl.setText(String.valueOf(errorCount) + " " + KETMessageService.service.getString("e3ps.bom.bom_editor_message", "00263")/* 에러 */
	        + ", ");
	warningLbl
	        .setText(String.valueOf(warningCount) + " " + KETMessageService.service.getString("e3ps.bom.bom_editor_message", "00080")/* 경고 */);
    }

    private void loadWarningMessage() {
	String message = messageRegistry.getString("valid8");
