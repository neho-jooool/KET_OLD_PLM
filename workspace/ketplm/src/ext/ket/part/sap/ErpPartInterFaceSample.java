...
import e3ps.sap.ErpPartDieDTO;
import e3ps.sap.ErpPartDieEnum;
import e3ps.sap.ErpPartDieHalbDTO;
import e3ps.sap.ErpPartDieHalbEnum;
import e3ps.sap.ErpPartMoldDTO;
import e3ps.sap.ErpPartMoldEnum;
import e3ps.sap.ErpPartProdDTO;
...
final public class ErpPartInterFaceSample {

	private static final String SPACE = " ";
	private static final String ECO_NO_ERP_FIELD = "I_ECONO";
	private static final String[] COLOR_ARRAY = { "BLACK", "BLUE", "BROWN", "GRAY", "D/GRAY", "L/GRAY", "GREEN", "L/GREEN", "IVORY", "JADE", "MAGENTA", "NATURAL", "ORANGE", "RED",
			"YELLOW", "기타", "WHITE", "LIGHT PINK", "SKY BLUE" };
	...
	public Map sendPartInfoToErp(List<WTPart> iPartList, String ecoNo, Map<String, String> workerNameMap) {
		...
		return sendPartInfoToErp(iPartList, ecoNo, workerNameMap, isOnlyInwork);
	}
	...
	public Map sendPartInfoToErp(List<WTPart> iPartList, String ecoNo, Map<String, String> workerNameMap, boolean isOnlyInwork) {
		...
		try {
			...
			for (WTPart wtPart : partList) {
				if (!isOnlyInwork || (isOnlyInwork && State.INWORK == wtPart.getLifeCycleState())) {
					...
					if (!handler.existErpPart(wtPart.getNumber())) {
						if (PartUtil.isProductType(partType)) {
							Kogger.debug(getClass(), "###### 1. erpIf part CREATE General :" + wtPart.getNumber() + " : " + wtPart.getVersionDisplayIdentifier().toString() + " : "
									+ CommonUtil.getOIDString(wtPart));
							partErpCreateSendProdList.add(wtPart);
						} else {
							Kogger.debug(getClass(), "###### 2. erpIf part CREATE Die Mold :" + wtPart.getNumber() + " : " + wtPart.getVersionDisplayIdentifier().toString() + " : "
									+ CommonUtil.getOIDString(wtPart));
						...
						if (PartUtil.isProductType(partType)) {
							Kogger.debug(getClass(), "###### 3. erpIf part Modify General :" + wtPart.getNumber() + " : " + wtPart.getVersionDisplayIdentifier().toString() + " : "
									+ CommonUtil.getOIDString(wtPart));
							partErpModifySendProdList.add(wtPart);
						} else {
							Kogger.debug(getClass(), "###### 4. erpIf part Modify Die Mold :" + wtPart.getNumber() + " : " + wtPart.getVersionDisplayIdentifier().toString() + " : "
									+ CommonUtil.getOIDString(wtPart));
							if ("D".equals(partType)) {
					...
					Kogger.debug(getClass(), "###### '진행중'이 아닌 WTPart는 대상에서 제외 :" + wtPart.getNumber() + " : " + wtPart.getLifeCycleState());
		...
		try {
			...
			if (erpType == CREATE_GENERAL || erpType == MODIFY_GENERAL) {
				...
				if (prodList.size() > 0) {
					...
					for (ErpPartProdDTO dto : prodList) {
						table.appendRow();
						for (ErpPartProdEnum erpPartProdEnum : prodEnumList) {
							Kogger.debug(getClass(), "ErpField : " + erpPartProdEnum.getErpField() + " PLM Attr: " + erpPartProdEnum.getAttrCode());
							Kogger.debug(getClass(), "ErpValue : " + checkErpValue(BeanUtils.getProperty(dto, erpPartProdEnum.getAttrCode())));
							table.setValue(checkErpValue(BeanUtils.getProperty(dto, erpPartProdEnum.getAttrCode())), erpPartProdEnum.getErpField());
						}
					}
					...
					for (int i = 0; i < numRows; i++) {
						...
						Kogger.debug(getClass(), "MATNR<<<<<< " + MATNR + ", ZSTAT<<<<<< " + ZSTAT + ", ZRMSG<<<<<< " + ZRMSG);
					...
					if (eReturn != null && "S".equals((eReturn.toUpperCase()))) {
						Kogger.debug(getClass(), "######## SUCCESS part CREATE ");
						success = true;
					} else {
						Kogger.debug(getClass(), "######## FAIL part CREATE ");
						success = false;
				...
				if (dieList.size() > 0 || moldList.size() > 0) {
					...
					for (ErpPartDieDTO dto : dieList) {
						...
						for (ErpPartDieEnum erpPartDieEnum : dieEnumList) {
							Kogger.debug(getClass(), "ErpField : " + erpPartDieEnum.getErpField() + " PLM Attr: " + erpPartDieEnum.getAttrCode());
							Kogger.debug(getClass(), "ErpValue : " + checkErpValue(BeanUtils.getProperty(dto, erpPartDieEnum.getAttrCode())));
							dieTable.setValue(checkErpValue(BeanUtils.getProperty(dto, erpPartDieEnum.getAttrCode())), erpPartDieEnum.getErpField());
						}
					}
					...
					for (ErpPartDieHalbDTO dto : dieHalbList) {
						...
						for (ErpPartDieHalbEnum erpPartDieHalbEnum : dieHalbEnumList) {
							Kogger.debug(getClass(), "ErpField : " + erpPartDieHalbEnum.getErpField() + " PLM Attr: " + erpPartDieHalbEnum.getAttrCode());
							Kogger.debug(getClass(), "ErpValue : " + checkErpValue(BeanUtils.getProperty(dto, erpPartDieHalbEnum.getAttrCode())));
							mapTable.setValue(checkErpValue(BeanUtils.getProperty(dto, erpPartDieHalbEnum.getAttrCode())), erpPartDieHalbEnum.getErpField());
						}
					}
					...
					for (ErpPartMoldDTO dto : moldList) {
						...
						for (ErpPartMoldEnum erpPartMoldEnum : moldEnumList) {
							Kogger.debug(getClass(), "ErpField : " + erpPartMoldEnum.getErpField() + " PLM Attr: " + erpPartMoldEnum.getAttrCode());
							Kogger.debug(getClass(), "ErpValue : " + checkErpValue(BeanUtils.getProperty(dto, erpPartMoldEnum.getAttrCode())));
							moldTable.setValue(checkErpValue(BeanUtils.getProperty(dto, erpPartMoldEnum.getAttrCode())), erpPartMoldEnum.getErpField());
						}
					}
					...
					if (eReturn != null && "S".equals((eReturn.toUpperCase()))) {
						Kogger.debug(getClass(), "######## SUCCESS part MODIFY ");
						success = true;
					} else {
						Kogger.debug(getClass(), "######## FAIL part MODIFY ");
						success = false;
						returnMap.put(ERP_ERROR_MSG, StringUtil.checkNull(eMsg));
						return success;
					}
	...
	private void setProdPartTable(WTPart wtPart, Map<String, String> workerNameMap, boolean create) throws Exception {
		...
		Kogger.debug(getClass(), "####### part General :" + wtPart.getNumber() + " : " + wtPart.getVersionDisplayIdentifier().toString() + " : " + CommonUtil.getOIDString(wtPart));
		...
		if (claz != null) {
			if (partTypeEnum == PartTypeEnum.FERT) {
		...
		setValue(dto, erpProdGroupCode, ErpPartProdEnum.ErpProdGroupCode);
		...
		if (materialPartSpecEnumList != null && materialPartSpecEnumList.size() > 0) {
			for (PartSpecEnum partSpecEnum : materialPartSpecEnumList) {
				if (partSpecEnum == PartSpecEnum.SpMaterialInfo) {
		...
		String workerName = (workerNameMap != null) ? workerNameMap.get(master.getNumber()) : "관리자";
	...
	private void setDiePartTable(WTPart wtPart, boolean create) throws Exception {
		...
		Kogger.debug(getClass(), "####### part Die :" + wtPart.getNumber() + " : " + wtPart.getVersionDisplayIdentifier().toString() + " : " + CommonUtil.getOIDString(wtPart));
		...
		setValue(dto, spObjectiveCT, ErpPartDieEnum.SpObjectiveCT);

		// Cavity
		String spCavityStd = PartSpecGetter.getPartSpecWithType(wTPartTypeInfo, PartSpecEnum.SpCavityStd);
		setValue(dto, spCavityStd, ErpPartDieEnum.SpCavityStd);
		...
		List<KETHalbPartDieSetPartLink> linkList = query.queryForListLinkByRoleB(KETHalbPartDieSetPartLink.class, WTPartMaster.class, master);
	...
	private void setMoldPartTable(WTPart wtPart, boolean create) throws Exception {
		...
		Kogger.debug(getClass(), "#######  part Mold :" + wtPart.getNumber() + " : " + wtPart.getVersionDisplayIdentifier().toString() + " : " + CommonUtil.getOIDString(wtPart));
	...
	private void setValue(ErpPartDieDTO dto, String value, ErpPartDieEnum erpPartDieEnum) throws Exception {
		...
		if (erpPartDieEnum.isEssential() && StringUtils.isEmpty(value)) {
			errorLogList.add("## Essential Check Error : DieNo:" + dto.getWtpartNumber() + " - " + erpPartDieEnum.getAttrCode() + " is empty !! ");
		}

		if (value != null && erpPartDieEnum.getDataLength() < getDataLength(value)) {
			errorLogList.add("## Length Check Error : DieNo:" + dto.getWtpartNumber() + " - " + erpPartDieEnum.getAttrCode() + " length is over ");
	...
	private void setValue(ErpPartDieHalbDTO dto, String value, ErpPartDieHalbEnum erpPartDieHalbEnum) throws Exception {
		...
		if (erpPartDieHalbEnum.isEssential() && StringUtils.isEmpty(value)) {
			errorLogList.add("## Essential Check Error : Die&HalbNo:" + dto.getDieNumber() + " - " + erpPartDieHalbEnum.getAttrCode() + " is empty !! ");
		}

		if (value != null && erpPartDieHalbEnum.getDataLength() < getDataLength(value)) {
			errorLogList.add("## Length Check Error : DieNo:" + dto.getDieNumber() + " - " + erpPartDieHalbEnum.getAttrCode() + " length is over ");
			errorLogList.add("## Value :=>" + value + "<=");
	...
	private void setValue(ErpPartMoldDTO dto, String value, ErpPartMoldEnum erpPartMoldEnum) throws Exception {
		...
		if (erpPartMoldEnum.isEssential() && StringUtils.isEmpty(value)) {
			errorLogList.add("## Essential Check Error : MoldNo:" + dto.getWtpartNumber() + " - " + erpPartMoldEnum.getAttrCode() + " is empty !! ");
		}

		if (value != null && erpPartMoldEnum.getDataLength() < getDataLength(value)) {
			errorLogList.add("## Length Check Error : DieNo:" + dto.getWtpartNumber() + " - " + erpPartMoldEnum.getAttrCode() + " length is over ");
			errorLogList.add("## Value :=>" + value + "<=");
	...
	private void setValue(ErpPartProdDTO dto, String value, ErpPartProdEnum erpPartProdEnum) throws Exception {
		...
		if (erpPartProdEnum.isEssential() && StringUtils.isEmpty(value)) {
			errorLogList.add("## Essential Check Error : ProdNo:" + dto.getWtpartNumber() + " - " + erpPartProdEnum.getAttrCode() + " is empty !! ");
		}

		if (value != null && erpPartProdEnum.getDataLength() < getDataLength(value)) {
			errorLogList.add("## Length Check Error : DieNo:" + dto.getWtpartNumber() + " - " + erpPartProdEnum.getAttrCode() + " length is over ");
			errorLogList.add("## Value :=>" + value + "<=");
