...
<table border="0" cellspacing="0" cellpadding="0" width="180" height="100%" >
	<tr>
		<td>
	...
	tree.treeSelectionListener = function(node, event){
		...
		*/
		...
		if(true){//if(node.depth !=0){
			target.document.location.href = "/plm/servlet/e3ps/NumberCodeMgtListServlet?command=<%=command%>&mode=<%=mode%>&invokeMethod=<%=invokeMethod%>&displayField=<%=displayField%>&valueField=<%=valueField%>&codetype=<%=rtnCodeType%>&parentOid="+sNodeOid
