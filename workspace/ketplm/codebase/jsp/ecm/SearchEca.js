...
//자료 검색
function doSearch(){
  document.forms[0].cmd.value = "searchEca";
...
function window.confirm(str) {
  execScript('n=msgbox("활동계획 종류선택(예:도면, 아니오:BOM, 취소:표준품)", "3")', "vbscript");
  return n;
}
//상세조회
function viewDetail(oid){
  var form = document.forms[0];
  var sel = confirm("활동계획 종류선택(1:도면, 2:BOM, 3:표준품)")
  if(sel == 6) {
    form.ecaType.value = "1";
  } else if(sel == 7) {
    form.ecaType.value = "2";
  } else if(sel == 2) {
    form.ecaType.value = "3";
  } else {
    return;
  }
  ...
  {
    url = "/plm/servlet/e3ps/MoldEcoChangeServlet";
  }
...
function popupRelEco() {
  ...
  attache = window.showModalDialog(url,window,"help=no; resizable=yes; status=no; scroll=no; dialogWidth=725px; dialogHeight:520px; center:yes");
...
//부품검색팝업 호출
function popupPart() {
  var url="/plm/servlet/e3ps/PartServlet?cmd=openSearchPopup&mode=one";
...
function selectedPart(objArr) {
  ...
  var trArr = objArr[0];
  var form = document.forms[0];
  form.partOid.value = trArr[0];
  form.partNo.value = trArr[1];
  form.partName.value = trArr[2];
}
