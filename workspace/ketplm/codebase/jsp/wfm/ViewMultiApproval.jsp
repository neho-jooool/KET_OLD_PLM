...
<%
    ...
    if (multiReq != null) {
		...
		if (currentUser.equals(multiReq.getCreator()))
		    isMine = true;
		desc = ParamUtil.checkStrParameter(multiReq.getDescription(), "");
		multiState = multiReq.getState().getState().getDisplay();
		if (multiReq.getState().getState().equals(State.toState("INWORK")) || multiReq.getState().getState().equals(State.toState("REWORK"))) {
		    availableState = true;
		}
		if (multiReq.getState().getState().equals(State.toState("APPROVED")))
		    lastApprover = WorkflowSearchHelper.manager.getLastApprover(multiReq).getFullName();
    }
...
function deleteMulti(){
    ...
    function pageSetting(sel) {
        document.forms[0].action = "ViewMultiApproval.jsp";
        document.forms[0].submit();
    }
</script>
<body>
            ...
            <%
                ...
                if (vec != null) {
            		...
            		for (index = size - ((cpage - 1) * perpage); index >= maxValue + 1; index--) {
            		    ...
            		    String epmVersion = "";
            		    String epmState = "";
            		    boolean availableStateEPM = true;
            		    if (!epmDoc.getState().getState().equals(State.toState("INWORK")) && !epmDoc.getState().getState().equals(State.toState("REWORK"))) {
            			availableStateEPM = false;
            		    }
            		    if (KETObjectUtil.getVersion(epmDoc).equals("0"))
            			epmVersion = KETObjectUtil.getVersion(epmDoc);
            		    else
            			epmVersion = "<font color=\"red\">" + KETObjectUtil.getVersion(epmDoc) + "</font>";
            		    if (availableState && !availableStateEPM)
            			epmState = "<font color=\"red\">" + epmDoc.getState().getState().getDisplay(messageService.getLocale()) + "</font>";
            		    else
            			epmState = epmDoc.getState().getState().getDisplay(messageService.getLocale());
            %>
            <tr>
              <td class="tdwhiteM"><%=index%></td>
              <td class="tdwhiteL" title="<%=epmDoc.getNumber()%>"><a href="javascript:doViewEPM('<%=epmDoc.toString()%>');"><nobr
                    style="text-overflow:ellipsis;overflow:hidden;width:130;cursor:hand;"
                  ><%=epmDoc.getNumber()%></nobr></a></td>
              ...
              <td class="tdwhiteM"><%=epmVersion%></td>
              <td class="tdwhiteM0"><%=epmState%></td>
            </tr>
            <%
                }
                }
 ...
 %>
          <table border="0" cellspacing="0" cellpadding="0" width="100%">
            <tr>
              <td class="small" align="left">(<%=messageService.getString("e3ps.message.ket_message", "02499")%><%--전체페이지--%> : <%=totalPage%>)
              </td>
              <td><table border="0" align="center" cellpadding="0" cellspacing="0">
                  <tr>
                    ...
                    <%
                        if (start > 1) {
                    %><td width="20" align="center"><a href="ViewMultiApproval.jsp?oid=<%=oid%>&cpage=1&perpage=<%=perpage%>" class='small'><img src='../../portal/images/btn_arrow4.gif'
                        style='border: 0'
                      ></a></td>
                    <%
                        }
                    ...
                    <td width="20" align="center"><a href="ViewMultiApproval.jsp?oid=<%=oid%>&cpage=<%=start - 1%>&perpage=<%=perpage%>" class='small'><img
                        src='../../portal/images/btn_arrow3.gif' style='border: 0'
                      ></a></td>
                    <td width="1" bgcolor="#dddddd"></td>
                    <%
                        }
                    ...
                    %>
                    <td style="padding: 2 8 0 7; cursor: hand" onMouseOver="this.style.background='#ECECEC'" OnMouseOut="this.style.background=''" class="nav_on"><font color=777777> <%
     if (i == cpage) {
 ...
 %>
                    ...
                    <td width="20" align="center"><a href="ViewMultiApproval.jsp?oid=<%=oid%>&cpage=<%=end + 1%>&perpage=<%=perpage%>" class='small'><img
                        src='../../portal/images/btn_arrow1.gif' style='border: 0'
                      ></a></td>
                    <%
                        }
                    %>
                    ...
                    <td width="20" align="center">
                      <%
                          if (end < totalPage) {
                      %><a href="ViewMultiApproval.jsp?oid=<%=oid%>&cpage=<%=totalPage%>&perpage=<%=perpage%>" class='small'><img src='../../portal/images/btn_arrow2.gif' style='border: 0'></a>
                    </td>
                    <%
                        }
                    %>
              ...
              <td class="small" align="right">(<%=messageService.getString("e3ps.message.ket_message", "02496")%><%--전체개수--%> : <%=totalSize%>)
              </td>
            </tr>
          </table> <%
          }
