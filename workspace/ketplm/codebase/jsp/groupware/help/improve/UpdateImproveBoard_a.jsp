...
<!--
  function modify(){
    ...
    showProcessing();
    disabledAllBtn();
    <%-- form01.action = "/plm/servlet/e3ps/ManageImproveBoardServlet?command=update&from=<%=from %>"; --%>
    form01.action = "/plm/servlet/e3ps/UpgradeBoardServlet?command=update&from=<%=from %>";
    form01.submit();
  }
...
//<![CDATA[
  ...
  $(document).ready(function(){
	  ...
	  $('[name=division]').change(function(){
		  if($(this).val() == 'SYSERR'){
			  $('[name=managerName]').val('관리자');
			  $("#causeTR").show();
			  $("#handleResultTR").show();
		  }else{
	          $('[name=managerName]').val("<%=data.getWriter()%>");      
	          $("#causeTR").hide();
	          $("#handleResultTR").hide();
	      }
	   });
...
<input type="hidden" name="poid" value="<%=qdata.getOid() %>">
...
<!-- 이노디터에서 작성된 내용을 보낼 Form Start (Answer) -->
    ...
    <!-- 이 부분은 배경색 또는 배경이미지 버튼을 사용하는 경우에만 해당 -->
<!-- 이노디터에서 작성된 내용을 보낼 Form End (Answer) -->
        ...
        <tr>
            <td>
                ...
                <tr>
                    ...
                    <td class="font_01"><%=messageService.getString("e3ps.message.ket_message", "00426") %><%--Q&amp;A 답변 수정--%></td>
       ...
       <tr>
        ...
        <tr>
            <td class="tdwhiteL0" colspan="6">
                ...
                <div id="divViewQ" style="width:750px;border:0px;" class="outline"></div>
        ...
        <table style="width: 100%;">
        <tr>
        	<td class="tdblueL" style="width: 120px;"><%=messageService.getString("e3ps.message.ket_message", "00969") %><%--구분--%><span class="red">*</span></td>
            <td class="tdwhiteL" style="width: 270px;">
                <select name="division" id="division" class="fm_jmp" style="width:270px;">
                ...
                parameter2.put("codeType", "IMPROVEITEM");
                List<Map<String, Object>> classificationNumCode = NumberCodeHelper.manager.getNumberCodeList(parameter2);
                ...
                %>
                    <option value="<%=code %>" <%=(data.getDivision()!=null&&data.getDivision().equals(code))?"selected":"" %>><%=value%></option>
                <%
                }
                %>
                </select>
            ...
            <td class="tdwhiteL0" style="width: 270px;">
              <select name="state" id="state" class="fm_jmp" style="width:98%">
                ...
                for ( int i=0; i<stateNumCode.size(); i++ ) {
                  ...
                  if (ImproveBoardData.STATE_REGISTERED.equals(code) == false) {
                %>
                    <option value="<%=code %>" <%=(data.getState()!=null&&data.getState().equals(code))?"selected":"" %>><%=value%></option>
                <%
                  }
                }
                %>
        ...
        <td width="120" class="tdblueL"><%=messageService.getString("e3ps.message.ket_message", "01205")%><%--담당자--%><span class="red">*</span></td>
            <td class="tdwhiteL" colspan="3">
                <input id="managerName" type="text" name="managerName" value="<%=data.getManager() %>" class="txt_fieldRO" readonly style="width: 30%">
        ...
        <%-- <tr>
            <td class="tdblueL"><%=messageService.getString("e3ps.message.ket_message", "02431") %>작성자</td>
            <td class="tdwhiteL">
                <%=data.getWriter() %><input type="hidden" name="writer" value="<%=data.getWriter() %>" />
            </td>
            <td class="tdblueL"><%=messageService.getString("e3ps.message.ket_message", "02425") %>작성부서</td>
            <td class="tdwhiteL0">
                <%=data.getDepartment() %><input type="hidden" name="department" value="<%=data.getDepartment() %>" />
            </td>
        </tr> --%>
        ...
        <tr>
        	<td class="tdblueL"   style="width: 120px;"><%=messageService.getString("e3ps.message.ket_message", "02428") %><%--작성일--%></td>
            <td class="tdwhiteL" style="width: 270px;">
                <%=data.getWriteDate() %><input type="hidden" name="writeDate" value="<%=data.getWriteDate() %>" />
            </td>
        ...
        <tr id="causeTR" style="display:none;">
            <td class="tdblueL" style="width: 120px;"><%=messageService.getString("e3ps.message.ket_message", "03434") %><%--원인--%></td>
            <td colspan="3"  style="width: 270px;" class="tdwhiteL0"><textarea name="cause" style='overflow: auto; width:98%; height:50px; padding: 10;' class="fm_area" ><%=data.getCause() %></textarea></td>
        </tr>

        <tr id="handleResultTR" style="display:none;">
            <td class="tdblueL" style="width: 120px;"><%=messageService.getString("e3ps.message.ket_message", "02777") %><%--처리내용--%></td>
            <td colspan="3"  style="width: 270px;" class="tdwhiteL0"><textarea name="handleResult" style='overflow: auto; width:98%; height:50px; padding: 10;' class="fm_area" ><%=data.getHandleResult() %></textarea></td>
        </tr>
        <tr>
            <td class="tdblueL"><%=messageService.getString("e3ps.message.ket_message", "02796") %><%--첨부파일--%></td>
