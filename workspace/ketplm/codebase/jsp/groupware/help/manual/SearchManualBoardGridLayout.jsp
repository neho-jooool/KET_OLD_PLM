...
<%
  int[] w = new int[8];
  w[0] = 40; // DocNumber
  w[1] = 100; // Classification
  w[2] = 100; // Classification1
  w[3] = 100; // Classification2
  
  w[5] = 90; // Writer
  w[6] = 100; // WriteDate
  w[7] = 50; // ReadCount
  w[4] = 760 - (w[0]+w[1]+w[2]+w[3]+w[5]+w[6]+w[7]); // Title
%>
    <LeftCols>
        ...
        <C Name='Classification' Width='<%=w[1] %>' Align='Center' CanSort='1' CanEdit='0' Type='Text' MinWidth='<%=colMinWidth%>' ExportStyle='text-align:center;vertical-align:middle'/>
        <C Name='Classification1' Width='<%=w[2] %>' Align='Center' CanSort='1' CanEdit='0' Type='Text' MinWidth='<%=colMinWidth%>' ExportStyle='text-align:center;vertical-align:middle'/>
        <C Name='Classification2' Width='<%=w[3] %>' Align='Center' CanSort='1' CanEdit='0' Type='Text' MinWidth='<%=colMinWidth%>' ExportStyle='text-align:center;vertical-align:middle'/>
        <C Name='Title'          RelWidth='50' Width='<%=w[4] %>' Align='Left'   CanSort='1' CanEdit='0' Type='Text' MinWidth='<%=colMinWidth%>' ExportStyle='text-align:left;vertical-align:middle'/>
    </LeftCols>

    <Cols>
        <C Name='Writer'         Width='<%=w[5] %>' Align='Center' CanSort='1' CanEdit='0' Type='Text' MinWidth='<%=colMinWidth%>' ExportStyle='text-align:center;vertical-align:middle'/>
        <C Name='WriteDate'      Width='<%=w[6] %>' Align='Center' CanSort='1' CanEdit='0' Type='Text' MinWidth='<%=colMinWidth%>' ExportStyle='text-align:center;vertical-align:middle'/>
        <C Name='ReadCount'      Width='<%=w[7] %>' Align='Center' CanSort='1' CanEdit='0' Type='Int'  MinWidth='<%=colMinWidth%>' ExportStyle='text-align:center;vertical-align:middle'/>
    </Cols>

    <Header CanDelete='0' Align='Center'  Spanned='1'
            DocNumber     ='<%=messageService.getString("e3ps.message.ket_message", "00342") %><%--No--%>'
            Classification='<%=messageService.getString("e3ps.message.ket_message", "00969") %><%--구분--%>'
            Classification1='<%=messageService.getString("e3ps.message.ket_message", "01606") %><%--분류--%>' Classification1Span='2'
            Classification2='<%=messageService.getString("e3ps.message.ket_message", "01606") %><%--분류--%>2'
