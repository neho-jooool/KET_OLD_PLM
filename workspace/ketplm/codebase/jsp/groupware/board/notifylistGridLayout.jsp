...
<%
  ...
  w[6] = 90; // Writer
  w[7] = 50; // ReadCount
  w[2] = 750 - (w[0]+w[1]+w[3]+w[4]+w[5]+w[6]+w[7]); // Title
%>
    <LeftCols>
        ...
        <C Name='AttachIcon'  Width='<%=w[1] %>' Align='Center' CanSort='0' CanEdit='0' Type='Text' MinWidth='<%=colMinWidth%>' ExportStyle='text-align:center;vertical-align:middle' CanExport="0"/>
        <C Name='Title'       RelWidth='50' Width='<%=w[2] %>' Align='Left'   CanSort='1' CanEdit='0' Type='Text' MinWidth='<%=colMinWidth%>' ExportStyle='text-align:left;vertical-align:middle'/>
    </LeftCols>

    <Cols>
        <C Name='WriteDate'   Width='<%=w[3] %>' Align='Center' CanSort='1' CanEdit='0' Type='Text' MinWidth='<%=colMinWidth%>' ExportStyle='text-align:center;vertical-align:middle'/>
        <C Name='LimitDate'   Width='<%=w[4] %>' Align='Center' CanSort='1' CanEdit='0' Type='Text' MinWidth='<%=colMinWidth%>' ExportStyle='text-align:center;vertical-align:middle'/>
        <C Name='WriterDept'  Width='<%=w[5] %>' Align='Center' CanSort='1' CanEdit='0' Type='Text' MinWidth='<%=colMinWidth%>' ExportStyle='text-align:center;vertical-align:middle'/>
        <C Name='Writer'      Width='<%=w[6] %>' Align='Center' CanSort='1' CanEdit='0' Type='Text' MinWidth='<%=colMinWidth%>' ExportStyle='text-align:center;vertical-align:middle'/>
        <C Name='ReadCount'   Width='<%=w[7] %>' Align='Center' CanSort='1' CanEdit='0' Type='Text' MinWidth='<%=colMinWidth%>' ExportStyle='text-align:center;vertical-align:middle'/>
    </Cols>

    <Header CanDelete='0' Align='Center'
            DocNo     ='<%=messageService.getString("e3ps.message.ket_message", "00342") %><%--No--%>'
            AttachIcon='<%=messageService.getString("e3ps.message.ket_message", "02794") %><%--첨부--%>'
            Title     ='<%=messageService.getString("e3ps.message.ket_message", "02524") %><%--제목--%>'
            WriteDate ='<%=messageService.getString("e3ps.message.ket_message", "01335") %><%--등록일--%>'
            LimitDate ='<%=messageService.getString("e3ps.message.ket_message", "00741") %><%--게시 기한일--%>'
            WriterDept='<%=messageService.getString("e3ps.message.ket_message", "01332") %><%--등록부서--%>'
            Writer    ='<%=messageService.getString("e3ps.message.ket_message", "01337") %><%--등록자--%>'
