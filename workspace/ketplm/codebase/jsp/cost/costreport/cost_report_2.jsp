...
<%
    ...
    for(int j=0; j < P_count; j++){
        ...
        suYearItem = (Hashtable)reportSuYearList.get(0);
            ...
            //out.println("pk_cw_array[j][p][q]=== :"+pk_cw_array[j][p][q] +"<br>");
    ...
    for(int j=0; j<P_count; j++){
        for(p=1;p<=Integer.parseInt(StringUtil.checkNullZero(case_count_1[j]));p++){
            ...
            //out.println("pkcw: "+pk_cw_array[s][d][b]+"<br>");
            ...
            //out.println(pk_cw_array[s][d][b]+" "+group_no[j][0][0]+"-"+k+"<br>");
    ...
    for(int j=0;j<P_count;j++){
        ...
        for(p=1;p<=Integer.parseInt(StringUtil.checkNullZero(case_count_1[j]));p++){
            for(q=1;q<=Integer.parseInt(StringUtil.checkNullZero(case_count_2[j][p]));q++){
                ...
                if(reportPkcwGroupNoList.size() > 0){
                    pkcwGroupNoItem = (Hashtable)reportPkcwGroupNoList.get(0);
                    if(pkcwGroupNoItem.size() > 0){
    ...
    for(int j=0;j<P_count;j++){
        for(p=0;p<=Integer.parseInt(StringUtil.checkNullZero(case_count_1[j]));p++){
            for(q=0;q<=Integer.parseInt(StringUtil.checkNullZero(case_count_2[j][p]));q++){
                if(pk_cw_array[j][p][q] != null && !StringUtil.checkNull(pk_cw_array[j][p][q]).equals("")){
                ...
                if(reportCostWorkList.size() > 0){
                    ...
                    //out.println("2번 => "+ j+" "+p+" "+q+" "+part_name[j][p][q]+" "+pk_cw_array[j][p][q]+"<br>");
...
<script language="javascript">
    function print_call(){
        window.open("/plm/jsp/cost/costreport/cost_report_2_print.jsp?visitor=<%=visitor%>&pk_cr_group=<%=pk_cr_group%>&cost_report_add=<%=cost_report_add%>&chk_list=<%=chk_list%>&table_row=<%=table_row%>&report_pk=<%=report_pk%>&page_no=3&DB_case_2=<%=DB_case_2%>&rev_no=<%=rev_no%>", "pop_300", "width=960,height=700,scrollbars=yes,resizable=no");
    }

    function back_call(){
<%if("3".equals(visitor) || "4".equals(visitor) || position.equals("본부장") || position.equals("부문장") || position.equals("사장")){%>
        this.close();
<%}else{%>
...
<%}%>
    ...
    **********************************************/
    function  report_call(){
        document.part_1.action = "/plm/jsp/cost/costreport/cost_report_1.jsp?visitor=<%=visitor%>&pk_cr_group=<%=pk_cr_group%>&cost_report_add=<%=cost_report_add%>&chk_list=<%=chk_list%>&table_row=<%=table_row%>&report_pk=<%=report_pk%>&rev_no=<%=rev_no%>&user_case_count=<%=user_case_count%>";
...
<%
                if(!"".equals(case_count_2[j][fk])){
                    for(int r=1; r<=Integer.parseInt(StringUtil.checkNullZero(case_count_2[j][fk]));r++){
%>
                            <tr>
                                <td bgcolor="<%=bg_co_txt%>" class="style1">&nbsp;</td>
                                <td bgcolor="#F2F5F9" class="style1">&nbsp;</td>
...
%>
                        ...
                        <table width="955" border="0" cellpadding="0" cellspacing="1" bgcolor="#333333">
                            <tr>
                                <td width="476" height="155" bgcolor="#FFFFFF"><textarea name="note_5" cols="75" rows="13" class="style9" readonly><%=note_5%></textarea></td>
                                <td width="476" bgcolor="#FFFFFF"><textarea name="note_5_1" cols="75" rows="13" class="style9" readonly><%=note_5_1%></textarea></td>
                            </tr>
                        </table>
                        <table width="906" border="0" cellpadding="0" cellspacing="1">
                            <tr>
                 ...
                 <tr>
      <td height="30" colspan="4" class="style8">
        <table width="104" border="0" cellpadding="0" cellspacing="1">
        <tr>
          <td height="30" align="right" valign="bottom"><img src="/plm/jsp/cost/acc_img/btn_print.gif" border="0" onClick="print_call();"style="cursor:pointer;"/></td>
          </tr>
          </table>	  </td>
  </tr>
            </table>
