...
<script language="JavaScript" for="WiseGridLME" event="Initialize()">
initLME();
</script>
<!--  	서버와의 통신이 정상적으로 완료되면 발생한다.   -->
<script language=javascript for="WiseGridLME" event="EndQuery()">
	GridEndQuery();
</script>
...
function doQuery(){
	...
	GridObjLME.SetParam("mode", "search_LME");

	//WiseGrid가 서버와 통신시에 데이터를 전달한다.
	GridObjLME.DoQuery(servlet_url);
}

/*신규등록*/
function doLineInsert() {
	var GridObjLME = document.WiseGridLME;
...
function doDelete(strColumnKey) {
	...
	var cnt = GridObjLME.GetRowCount()
		chkCount = 0;
	...
	for(var i=0; i<cnt; i++) {
		...
		if(GridObjLME.getCellValue("CRUD", i) != "NEW") {
			if(GridObjLME.GetCellValue("SELECTED", i) == "1")
				GridObjLME.DeleteRow(i);
		} else {
			if(GridObjLME.GetCellValue("SELECTED", i) == "1")
				GridObjLME.DeleteRow(i);
			i = i-1;
	...
	//GridObjLME.DeleteRow(GridObjLME.GetActiveRowIndex());
}
/*로우 체크 확인*/
function chkSelected() {
	chkCount = 0;

	for(i = 0; i < GridObjLME.GetRowCount(); i++) { //그리드 데이터의 로우수를 반환한다.

		if(GridObjLME.GetCellValue("SELECTED", i) == "1") //지정한 셀의 값을 가져온다.
			chkCount = chkCount + 1;
	}
...
function doSave() {
	...
	GridObjLME.SetParam("mode", "save_LME");

	//WiseGrid가 서버와 통신시에 데이터를 전달한
	GridObjLME.DoQuery(servlet_url, "CRUD");
}
...
function GridEndQuery() {
	...
	var mode = GridObjLME.GetParam("mode");

	if(mode == "search_LME") {
		if(GridObjLME.GetStatus() == "true") 	{// 서버에서 전송한 상태코드를 가져온다.
		} else	{
			var error_msg = GridObjLME.GetMessage(); // 서버에서 전송한 상태코드값이 false라면 에러메세지를 가져온다.
			alert(error_msg);
		}
	}
...
<body>
<form name="LMEList">
<table width="800" height="293" border="0" cellpadding="0" cellspacing="0">
  <tr>
    <td height="31">LMEDataBase</td>
  </tr>
  <tr>
  ...
  <tr>
    <td><script>initWiseGrid("WiseGridLME", "800", "600");</script></td>
  </tr>
</table>
</form>
</body>
