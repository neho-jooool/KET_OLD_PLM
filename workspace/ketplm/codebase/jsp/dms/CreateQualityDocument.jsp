    ...
    function setProject(returnValue){
        ...
        for ( var i = 0; i < objArr.length; i++ ) {
            ...
            if ( !projDuplicateCheck(proj_oid) ) {
                ...
                addProjectProductInfoAjax(proj_oid);
            }
            else {
...
%>
    ...
    function insertPartLine (){
        ...
        SearchUtil.selectPart("setPartInfoSbl","fromPage=SBL&partType=P&number="+number+"&projectNo="+proj_numbers);
    }
    /*
     아 오늘도 밤샜다.
     죽겠다.... tklee
    ...
    function chgFirstRegist(str){
        ...
        if ( fm.firstRegistrationStage.value != str ) {
            if ( str=="1" ) {
                ...
                $("#projectText").html("<%=messageService.getString("e3ps.message.ket_message", "03046")%><%--프로젝트--%> <span class='red'>*</span>");
            }
            else {
                fm.firstRegistrationStage1.checked = false;
                fm.firstRegistrationStage2.checked = true;
                $("#projectText").html("<%=messageService.getString("e3ps.message.ket_message", "03046")%><%--프로젝트--%>");
            }
...
//             parent.opener.document.location.reload();
    ...
    function valcheck () {
        ...
        if(checked_code!="PD303034"){
	        if(isNull(d.quiltyNo.value.replace(/ /gi,""))){
...
<script type="text/javascript">
    ...
    function setDevDocCategoryAjax(categoryCode, categoryCodeTxt){
        $.ajax({
            ...
            success : function(data) {
            	...
            	var isDRCheckSheet = data.isDRCheckSheet;
    ...
    $(document).ready(function() {
        ...
        $("[name=categoryCode]").change(function() {
            setDevDocCategoryAjax($('[name=categoryCode]').val(), $('[name=categoryCodeTxt]').val());
            for(var i=0;i < document.frm.categoryCode.length; i++){
                if(document.frm.categoryCode[i].checked == "" || document.frm.categoryCode[i].checked == null){
                }else{
                    checked_code = document.frm.categoryCode[i].value;
                    result_code = "";
                    if(checked_code == "PD303066"){
                    	result_code = "DF-";
                    }else if(checked_code == "PD303072"){
                    	result_code = "PF-";
                    }else if(checked_code == "PD303001"){
                    	result_code = "KIS-";
                    }else if(checked_code == "PD303017"){
                    	result_code = "CP-";
                    }else if(checked_code == "PD303034"){
                    	result_code = "KEQ-";
                    }else if(checked_code == "PD303046"){
                    	result_code = "KETS-";
                    }else if(checked_code == "PD303047"){
                    	result_code = "PFC-";
                    }
        ...
        $("[name=iFile0]").on("change", function(){
            //alert($("[name=documentName]").val().length);
...
<body>
  <form name="frm" method="post">
    ...
    <input type="hidden" name="analysisCode" value="">
          ...
          <table border="0" cellspacing="0" cellpadding="0" width="100%" style="table-layout: fixed;">
            ...
            <tr>
              ...
              <td colspan="3" class="tdwhiteL0">
                <input type="radio" name="categoryCode" value="PD303066">DFMEA
                <input type="radio" name="categoryCode" value="PD303072">PFMEA
                <input type="radio" name="categoryCode" value="PD303001">검사기준서
                <input type="radio" name="categoryCode" value="PD303017">관리계획서
                <input type="radio" name="categoryCode" value="PD303034">설비점검기준서
                <input type="radio" name="categoryCode" value="PD303046">작업표준서
                <input type="radio" name="categoryCode" value="PD303047">제조공정도</td>
            </tr>
            <tr>
			  <td class="tdblueL">품질문서번호</td>
			  <td colspan="3" class="tdwhiteL0">
			    <span id="quiltyTR"></span>
			    <input type="text" name="quiltyNo" class="txt_field" style="width: 15%">
			  </td>
            <tr>
            ...
            <tr>
              ...
              <td class="tdwhiteL"><input name="firstRegistrationStage1" <%if (outputOids != null) {%> disabled <%}%> type="radio" class="Checkbox" value="1" checked
                  onClick="javascript:chgFirstRegist('1')"
                ><%=messageService.getString("e3ps.message.ket_message", "00629")%><%--개발단계--%>&nbsp; <input name="firstRegistrationStage2" <%if (outputOids != null) {%> disabled <%}%> type="radio"
 ...
 %>
            ...
            <tr>
              ...
              <td rowspan="2" class="tdblueL" id="customerText"><%=messageService.getString("e3ps.message.ket_message", "00837")%><%--고객사--%></td>
            ...
            <tr>
              <td class="tdblueL" id="projectText"><%=messageService.getString("e3ps.message.ket_message", "03046")%><%--프로젝트--%></td>
              <td colspan="3" class="tdwhiteL0">
