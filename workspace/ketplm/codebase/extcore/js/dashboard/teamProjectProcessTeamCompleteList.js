var teamProjectProcessTeamCompleteList = {
	
	
	/**
	 * server paging grid
	 */
	createPaingGrid : function(departName,displayCode,startDate,endDate){
		this.grid = TreeGrid(CommonGrid.getPagingGridConfig({
			id : 'SampleSearchGrid',
			Sort : '-pjtName',
			perPageOnChange : 'javascript:teamProjectProcessTeamCompleteList.search(Value);',
			Data : {
				Url : '/plm/ext/dashboard/teamProjectProcessTeamCompleteListSetting?sortName=*Sort0',
				Method : 'POST',
				Format : 'Form',
				Timeout : 0,
				Param : {
			...
			Cols : [
				...
				Head :[
						...
						Toolbar : {
							...
							perPageOnChange:'javascript:teamProjectProcessCompleteList.search(Value);',
		...
		//row click event
		Grids.OnClick = teamProjectProcessTeamCompleteList.goView;
		
		//download page  event
		Grids.OnDownloadPage = function(grid,row){
			grid.Data.Page.Format = 'Form';
			grid.Data.Page.Method = 'Post';
			grid.Data.Page.Url = '/plm/ext/dashboard/teamProjectProcessTeamCompleteListSetting?sortName=*Sort0';
	...
	goView : function(grid,row,col,x,y){
		if(col != 'Panel' && row.oid){
			if(col == "pjtNo" && row.pjtTypeName == "검토"){
				...
				getOpenWindow2('/plm/jsp/project/MoldProjectView.jsp?oid='+row.mOid+"&popup=popup",'SampleUpdatePopup',800,600);
			}
		}
	},
