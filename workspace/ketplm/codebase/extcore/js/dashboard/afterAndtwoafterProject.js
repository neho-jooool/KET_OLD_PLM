var afterAndtwoafterProject = {
	
	/**
	 * server paging grid
	 */
	createPaingGrid : function(type,devType,year,month,pjtType){
		this.grid = TreeGrid(CommonGrid.getPagingGridConfig({
			id : 'SampleSearchGrid',
			Sort : '-pjtNo',
			perPageOnChange : 'javascript:afterAndtwoafterProject.search(Value);',
			Data : {
				Url : '/plm/ext/dashboard/afterAndtwoafterProjectList?sortName=*Sort0',
				Method : 'POST',
				Format : 'Form',
				Timeout : 0,
				Param : {
		...
		//row click event
		Grids.OnClick = afterAndtwoafterProject.goView;
		
		//download page  event
		Grids.OnDownloadPage = function(grid,row){
			grid.Data.Page.Format = 'Form';
			grid.Data.Page.Method = 'Post';
			grid.Data.Page.Url = '/plm/ext/dashboard/afterAndtwoafterProjectList?sortName=*Sort0';
