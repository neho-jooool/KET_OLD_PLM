var itemMoldListPopup = {
	...
	createPaingGrid : function(customer,carType,itemType){
		this.grid = TreeGrid(CommonGrid.getPagingGridConfig({
			id : 'SampleSearchGrid',
			Sort : '-pjtNo',
			perPageOnChange : 'javascript:itemMoldListPopup.search(Value);',
			Data : {
				Url : '/plm/ext/dashboard/itemMoldListPopupSetting?sortName=*Sort0',
				Method : 'POST',
				Format : 'Form',
				Timeout : 0,
				Param : {
		...
		//row click event
		Grids.OnClick = itemMoldListPopup.goView;
		
		//download page  event
		Grids.OnDownloadPage = function(grid,row){
			grid.Data.Page.Format = 'Form';
			grid.Data.Page.Method = 'Post';
			grid.Data.Page.Url = '/plm/ext/dashboard/itemMoldListPopupSetting?sortName=*Sort0';
