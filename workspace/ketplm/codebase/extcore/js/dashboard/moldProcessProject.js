var moldProcessProject = {
	...
	createPaingGrid : function(customer,carType,year){
		this.grid = TreeGrid(CommonGrid.getPagingGridConfig({
			id : 'SampleSearchGrid',
			Sort : '-stateState',
			perPageOnChange : 'javascript:moldProcessProject.search(Value);',
			Data : {
				Url : '/plm/ext/dashboard/moldProcessProjectList?sortName=*Sort0',
				Method : 'POST',
				Format : 'Form',
				Timeout : 0,
				Param : {
		...
		//row click event
		Grids.OnClick = moldProcessProject.goView;
		
		//download page  event
		Grids.OnDownloadPage = function(grid,row){
			grid.Data.Page.Format = 'Form';
			grid.Data.Page.Method = 'Post';
			grid.Data.Page.Url = '/plm/ext/dashboard/moldProcessProjectList?sortName=*Sort0';
