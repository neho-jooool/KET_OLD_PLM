...
<script type="text/javascript" src="/plm/extcore/js/dashboard/carTypeSingle.js"></script>
<jsp:useBean id="messageService" class="e3ps.common.message.KETMessageService" scope="session" />
<!-- EJS TreeGrid End -->

<body class="popup-background popup-space">
<div class="sub-title"><img src="/plm/portal/images/icon_3.png" /><%=messageService.getString("e3ps.message.ket_message", "07298") %><c:if test="${dashBoardDTO.itemType eq 'A'}">(<%=messageService.getString("e3ps.message.ket_message", "07266") %>)</c:if><c:if test="${dashBoardDTO.itemType eq 'B'}">(<%=messageService.getString("e3ps.message.ket_message", "00966") %>)</c:if></div>
...
$(document).ready(function(){
  ...
  //  alert("${dashBoardDTO.making}"+" "+"${dashBoardDTO.type}"+"  "+"${dashBoardDTO.moldTypeCheck}");
    
    carTypeSingle.createPaingGrid("${dashBoardDTO.customer}","${dashBoardDTO.carType}","${dashBoardDTO.devType}","${dashBoardDTO.itemType}");
});
