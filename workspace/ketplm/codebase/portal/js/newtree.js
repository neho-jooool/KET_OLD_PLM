...
function Tree(tid, rootName, dir, rootDesc)
{
    ...
    */
	this.iconDir = null;
	if ( dir == null ) {
		this.iconDir = "/eSolution/portal/icon/tree/win2000";
	} else {
		this.iconDir = dir;
	}
    ...
    * Root Node of tree
    */
   this.root = new TreeNode(rootName,rootDesc);
   /**
    * Preload icons
    */
   this.loadIcons = function()
   {
	  ...
	  this.ROOT_ICON = this.icons[10];
	  this.USER_ICON = this.icons[11];
   }
   this.loadIcons();
   /**
    * paint tree
   ...
   { 
	   var count = this.changedNodes.length;
	   if ( count >= 2 ) count = 2
	...
	this.leftSideForIE = function(node) {
		...
		if (node == this.root ) {
			...
			if(this.openedNode == node) {
				...
				renderString = renderString + "<SPAN style='color:" + fg + ";letter-spacing:-1px; background-color:" + bg + ";font-size:" + this.fontSize + ";font-family:" + this.fontName + ";cursor:hand'" + " onClick='nodes[" + node.index + "].tree.treeSelectionListener(nodes[" + node.index + "])'>"+"<IMG border='0' src='" + folderIcon.src + "' align='absmiddle'>" + node.name ;
			} else {
				var folderIcon = this.CLOSED_DEFAULT_FOLDER_ICON;
				if(node.icon != "") {
					folderIcon = new Image();
			...
			if(this.openedNode == node) {
				...
				if(node.get("userid")!=null){
						var userIcon = this.USER_ICON;
						userIcon = new Image();
						userIcon.src = this.iconDir + "/user_icon."+ this.iconExt;
						renderString = renderString + "<IMG border='0' src='" + userIcon.src + "' align='absmiddle'>";
				}else{
				var folderIcon = this.CLOSED_DEFAULT_FOLDER_ICON;
				if(node.icon != "") {
						folderIcon = new Image();
		...
		var nameValue = "";
		if(node.name.substring(0, 5) == "<FONT") nameValue = node.name.substring(21, node.name.length-7);
		else nameValue = node.name;

	
		renderString = renderString + "<SPAN style='color:" + fg + ";letter-spacing:-1px; background-color:" + bg + ";font-size:" + this.fontSize + ";font-family:" + this.fontName + ";cursor:hand'" + " onClick='nodes[" + node.index + "].tree.treeSelectionListener(nodes[" + node.index + "])' >" + node.name 
		
		
			
		}
		
		renderString = renderString + "</SPAN> ";
		
		if(node.isIssue){
		    var issueSrc = this.iconDir + "/warning.png";
			renderString = renderString + "<IMG border='0' src='" + issueSrc + "' valign='top' align='absmiddle'>";
		}
		

		if ( node.desc != null ) {
			renderString = renderString + "-<span style='font-size:8pt;letter-spacing:-1px;'><font color=red >"+node.desc+"</font></span>";
		}
		return renderString;
	}
    ...
    */
	this.leftSideForNE = function(node) {
		...
		if ( node.desc != null ) {			
			renderString = renderString + "-<font color=red>"+node.desc+"</font>";
		}
		return renderString;
	}
   /**
...
*/
   ...
   this.clone = function()
   {
      var newNode = new TreeNode(this.name,this.desc);
...
function popupShow(contentValue) {
    var oPopupBody = oPopup.document.body;
	oPopupBody.style.backgroundColor = "lightyellow";
	oPopupBody.style.border = "solid black 1px";    
    oPopupBody.innerHTML = "<table id=nodeName><tr><td><label>" + contentValue + "</label></td></tr></table>";
    oPopup.show(0, 0, 500, 0, document.body);

	var widthValue = oPopup.document.getElementById("nodeName").clientWidth;
	var heightValue = oPopup.document.getElementById("nodeName").clientHeight;
    oPopup.show(event.x+5, event.y+5, widthValue+3, heightValue+2, document.body);
}
function popupHide() {
	oPopup.hide();
}
