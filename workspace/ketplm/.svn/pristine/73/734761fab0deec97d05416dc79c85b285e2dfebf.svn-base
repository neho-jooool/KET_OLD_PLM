...
<script type="text/javascript" src="/plm/extcore/js/dashboard/moldReport.js"></script>
<jsp:useBean id="messageService" class="e3ps.common.message.KETMessageService" scope="session" />
<!-- EJS TreeGrid End -->
<body class="popup-background popup-space">
<div class="sub-title"><img src="/plm/portal/images/icon_3.png" /><%=messageService.getString("e3ps.message.ket_message", "07298") %> <c:if test="${dashBoardDTO.itemType eq ''}"></c:if><c:if test="${dashBoardDTO.itemType ne ''}">(${dashBoardDTO.itemType})</c:if></div>
...
$(document).ready(function(){
   ...
   // alert("${dashBoardDTO.programNo}");
  //  alert("${dashBoardDTO.customer}"+" "+"${dashBoardDTO.carType}"+"  "+"${dashBoardDTO.year}");
    
    moldReport.createPaingGrid("${dashBoardDTO.programNo}","${dashBoardDTO.making}","${dashBoardDTO.itemType}");
