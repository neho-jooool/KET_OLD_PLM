 ...
 * 
 */
public class PartFertNoGenerator extends AbsCodeGenerator {

    @Override
    public String generatePartNo(PartGenDTO dto, PartBaseDTO baseDto) throws Exception {
	if ("C".equals(dto.getDivisionFlag()) || "N".equals(dto.getDivisionFlag())) { // 자동차 OR KETN
    ...
    // 2(채번코드)+4(일련번호)+1(-)+2(색상-6 Connector류/도금-7 Terminal류)
	...
	if (USE_MAX) {
	    serial4No = getMaxSerial(prefix2No, "", 4, gubun1No, checkFlag, true);
	    if (StringUtils.isEmpty(serial4No))
		serial4No = SERIAL_4;
	    else
		serial4No = String.valueOf(Integer.parseInt(serial4No) + 1);
	} else {
	    serial4No = getMinSerial(prefix2No, "", 4, gubun1No, checkFlag, true, minNo);
	}

	if (isMaxOrOver(serial4No, 4))
	    throw new Exception("## 채번 Serial 한도 초과되었습니다.");

	// String partNo = prefix2No + getSerailNumberFormat(serial4No, 4) + endGubun1Plus3No;
	String partNo = erpCheckNumber(prefix2No, getSerailNumberFormat(serial4No, 4), endGubun1Plus3No);
	...
	// String partNo = prefix2No + getSerailNumberFormat(serial4No, 4) + platingAt1No + endGubun1Plus3No;
	String partNo = erpCheckNumber(prefix2No, getSerailNumberFormat(serial4No, 4), platingAt1No + endGubun1Plus3No);
