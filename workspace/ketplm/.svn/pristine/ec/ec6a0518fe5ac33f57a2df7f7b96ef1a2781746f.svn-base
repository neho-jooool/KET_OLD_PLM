...
<script language="JavaScript" for="WiseGridProces" event="Initialize()">
initProces();
</script>
<!--  	서버와의 통신이 정상적으로 완료되면 발생한다.   -->
<script language=javascript for="WiseGridProces" event="EndQuery()">
	GridEndQuery();
</script>
...
function doQuery(){
	...
	GridObjProces.SetParam("mode", "search_Proces");

	//WiseGrid가 서버와 통신시에 데이터를 전달한다.
	GridObjProces.DoQuery(servlet_url);
}

/*신규등록*/
function doLineInsert() {
	var GridObjProces = document.WiseGridProces;

	//그리드의 마지막 열에 빈 로우를 추가한다.
	GridObjProces.AddRow();
}
...
function doDelete(strColumnKey) {
	...
	var cnt = GridObjProces.GetRowCount()
		chkCount = 0;
	...
	for(var i=0; i<cnt; i++) {
		...
		if(GridObjProces.getCellValue("CRUD", i) != "NEW") {
			if(GridObjProces.GetCellValue("SELECTED", i) == "1")
				GridObjProces.DeleteRow(i);
		} else {
			if(GridObjProces.GetCellValue("SELECTED", i) == "1")
				GridObjProces.DeleteRow(i);
			i = i-1;
	...
	//GridObjProces.DeleteRow(GridObjProces.GetActiveRowIndex());
}
/*로우 체크 확인*/
function chkSelected() {
	chkCount = 0;

	for(i = 0; i < GridObjProces.GetRowCount(); i++) { //그리드 데이터의 로우수를 반환한다.

		if(GridObjProces.GetCellValue("SELECTED", i) == "1") //지정한 셀의 값을 가져온다.
			chkCount = chkCount + 1;
	}
...
function doSave() {
	...
	GridObjProces.SetParam("mode", "save_Proces");

	//WiseGrid가 서버와 통신시에 데이터를 전달한
	GridObjProces.DoQuery(servlet_url, "CRUD");
}
...
function GridEndQuery() {
	...
	var mode = GridObjProces.GetParam("mode");

	if(mode == "search_Proces") {
		if(GridObjProces.GetStatus() == "true") 	{// 서버에서 전송한 상태코드를 가져온다.
		} else	{
			var error_msg = GridObjProces.GetMessage(); // 서버에서 전송한 상태코드값이 false라면 에러메세지를 가져온다.
			alert(error_msg);
		}
	}
...
<body>
<form name="ProcesList">
<table width="360" height="293" border="0" cellpadding="0" cellspacing="0">
  <tr>
    <td height="31">가공비DataBase</td>
  </tr>
  <tr>
  ...
  <tr>
    <td><script>initWiseGrid("WiseGridProces", "360", "400");</script></td>
  </tr>
</table>
</form>
</body>
