var teamTaskTeamProcessData = {
	...
	createPaingGrid : function(displayCode,startDate,endDate,departName){
		this.grid = TreeGrid(CommonGrid.getPagingGridConfig({
			id : 'SampleSearchGrid',
			Sort : '-taskName',
			perPageOnChange : 'javascript:teamTaskTeamProcessData.search(Value);',
			Data : {
				Url : '/plm/ext/dashboard/teamTaskProcessTeamDataList?sortName=*Sort0',
				Method : 'POST',
				Format : 'Form',
				Timeout : 0,
				Param : {
		...
		//row click event
		Grids.OnClick = teamTaskTeamProcessData.goView;
		
		//download page  event
		Grids.OnDownloadPage = function(grid,row){
			grid.Data.Page.Format = 'Form';
			grid.Data.Page.Method = 'Post';
			grid.Data.Page.Url = '/plm/ext/dashboard/teamTaskProcessTeamDataList?sortName=*Sort0';
