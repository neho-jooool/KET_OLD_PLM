...
<%
  ...
  TemplateProject newProject = (TemplateProject)CommonUtil.getObject(oid);
...
<%  if(CommonUtil.isMember("PM_GROUP") || CommonUtil.isAdmin()){%>
  <!--tr>
    <td width="10" align="left" width="0" height=20 >
      <table border="0" cellspacing="0" cellpadding="0">
        <tr>
          <td class=fixLeft></td>
          <td ><input type=button class="btnTras" value='새로고침' id="button" onclick="javascript:reloadTree();"></td>
...
<SCRIPT>
  ...
  tree.treeSelectionListener = function(node, event){
    ...
    if ( node.get("nodeType") == "project" ) {
      target.document.location.href = "/plm/jsp/project/template/TemplateCompareProjectView.jsp?oid="+node.get("oid")+ "&oid=" + node.get("oldOid");
    } else if ( node.get("nodeType") == "task" ) {
      target.document.location.href = "/plm/jsp/project/template/TemplateCompareTaskView.jsp?oid="+node.get("oid") +
...
<%
  ...
  for(int i = 0; i < list.size(); i++){
    ...
    if(compareTask != null){
      ...
      Kogger.debug("compareTaskOid ===>" + compareTaskOid);
...
private String getTaskStateFont(DefaultProjectTreeNode node) {
  ...
  switch(result){
    case DefaultProjectTreeNode.ADD:
      tmpStr = " title='" + messageService.getString("e3ps.message.ket_message", "02868")/*추가된 태스크 입니다*/ + "'";
      fontStr ="<FONT color='#0033CC'>";
      break;
    case DefaultProjectTreeNode.DELTE:
      tmpStr = " title='" + messageService.getString("e3ps.message.ket_message", "01702")/*삭제된 태스크 입니다*/ + "'";
      fontStr ="<FONT color='#FF3300'>";
      break;
    case DefaultProjectTreeNode.MODIFY:
      tmpStr = " title='" + messageService.getString("e3ps.message.ket_message", "01949")/*수정된 태스크 입니다*/ + "'";
      fontStr ="<FONT color='#66FF33'>";
      break;
    case DefaultProjectTreeNode.DEFAULT:
      tmpStr = " title='변경 이력 없음'";
