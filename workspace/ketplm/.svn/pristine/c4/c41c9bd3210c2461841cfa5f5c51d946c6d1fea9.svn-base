package e3ps.bom.command.moveup;
...
public class MoveUpCmd extends AbstractAIFCommand
{
    ...
    public MoveUpCmd(JFrame frame, AbstractAIFUIApplication app)
    {
        this.app = app;
    }
    ...
    protected void executeCommand() throws Exception
    {
        try
        {
            ...
            userInfo = BOMBasicInfoPool.getUserName() + "(" + BOMBasicInfoPool.getUserEMail() + ")";
//            userInfo =  Utility.checkNVL(BOMBasicInfoPool.getUserDept()) + "/" + Utility.checkNVL(BOMBasicInfoPool.getUserName());
            ...
            if (parents.length != 1)
            {
                ...
                if(bomGubunFlag)
                {
                    // Parent Item Code 가 New 인 상태인 경우 Move Up 가능.
...
//                    MessageBox m = new MessageBox(app.getDesktop(), messageRegistry.getString("checkAdmin"), "Error", MessageBox.ERROR);
                    MessageBox m = new MessageBox(app.getDesktop(), "선택한 부품은 이동할 수 없습니다.", "Error", MessageBox.ERROR);
                    m.setVisible(true);
                    m.setModal(true);
                    return;
                }
            ...
            // 동일 레벨안에서 Move Up 가능.
            BOMTreeNode targetParentNode = (BOMTreeNode)nodes[0].getPreviousNode();
