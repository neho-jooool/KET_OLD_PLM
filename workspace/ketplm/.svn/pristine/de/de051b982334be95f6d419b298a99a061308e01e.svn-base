...
<script language="JavaScript" for="WiseGridDis" event="Initialize()">
initDis();
</script>
<!--  	서버와의 통신이 정상적으로 완료되면 발생한다.   -->
<script language=javascript for="WiseGridDis" event="EndQuery()">
	GridEndQuery();
</script>
...
function doQuery(){
	...
	GridObjDis.SetParam("mode", "search_Dis");

	//WiseGrid가 서버와 통신시에 데이터를 전달한다.
	GridObjDis.DoQuery(servlet_url);
}

/*신규등록*/
function doLineInsert() {
	var GridObjDis = document.WiseGridDis;
...
function doDelete(strColumnKey) {
	...
	var cnt = GridObjDis.GetRowCount()
		chkCount = 0;
	...
	for(var i=0; i<cnt; i++) {
		...
		if(GridObjDis.getCellValue("CRUD", i) != "NEW") {
			if(GridObjDis.GetCellValue("SELECTED", i) == "1")
				GridObjDis.DeleteRow(i);
		} else {
			if(GridObjDis.GetCellValue("SELECTED", i) == "1")
				GridObjDis.DeleteRow(i);
			i = i-1;
	...
	//GridObjDis.DeleteRow(GridObjDis.GetActiveRowIndex());
}
/*로우 체크 확인*/
function chkSelected() {
	chkCount = 0;

	for(i = 0; i < GridObjDis.GetRowCount(); i++) { //그리드 데이터의 로우수를 반환한다.

		if(GridObjDis.GetCellValue("SELECTED", i) == "1") //지정한 셀의 값을 가져온다.
			chkCount = chkCount + 1;
	}
...
function doSave() {
	...
	GridObjDis.SetParam("mode", "save_Dis");

	//WiseGrid가 서버와 통신시에 데이터를 전달한
	GridObjDis.DoQuery(servlet_url, "CRUD");
}
...
function GridEndQuery() {
	...
	var mode = GridObjDis.GetParam("mode");

	if(mode == "search_Dis") {
		if(GridObjDis.GetStatus() == "true") 	{// 서버에서 전송한 상태코드를 가져온다.
		} else	{
			var error_msg = GridObjDis.GetMessage(); // 서버에서 전송한 상태코드값이 false라면 에러메세지를 가져온다.
			alert(error_msg);
		}
	}
...
<table width="720" height="293" border="0" cellpadding="0" cellspacing="0">
  ...
  <tr>
    <td><script>initWiseGrid("WiseGridDis", "720", "600");</script></td>
  </tr>
</table>
</form>
</body>
