 ...
 *
 * <p>
 * Use the <code>newOEMProjectType</code> static factory method(s), not
 * the <code>OEMProjectType</code> constructor, to construct instances of
...
@GenAsPersistable(interfaces={Tree.class}, 
   ...
   @GeneratedProperty(name="oLevel", type=int.class),
   @GeneratedProperty(name="isSavePlan", type=boolean.class),
   @GeneratedProperty(name="isEcoCar", type=Boolean.class, initialValue="false",
      javaDoc="친환경여부"),
   @GeneratedProperty(name="isRunningChange", type=Boolean.class, initialValue="false",
   javaDoc="Running Change 여부"),
   @GeneratedProperty(name="oemType", type=OEMType.class,
      constraints=@PropertyConstraints(required=true))
   },
   foreignKeys={
   @GeneratedForeignKey(myRoleIsRoleA=false,
      foreignKeyRole=@ForeignKeyRole(name="maker", type=e3ps.common.code.NumberCode.class,
         constraints=@PropertyConstraints(required=true)),
      myRole=@MyRole(name="theOEMProjectType", cardinality=Cardinality.ONE))
   })
public class OEMProjectType extends _OEMProjectType {
    ...
    **/
   public static OEMProjectType newOEMProjectType()
            throws WTException {

      OEMProjectType instance = new OEMProjectType();
      instance.initialize();
      return instance;
   }
