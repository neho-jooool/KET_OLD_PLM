...
<script language="JavaScript" for="WiseGridCutt" event="Initialize()">
initCutt();
</script>
<!--  	서버와의 통신이 정상적으로 완료되면 발생한다.   -->
<script language=javascript for="WiseGridCutt" event="EndQuery()">
	GridEndQuery();
</script>
...
function doQuery(){
	...
	GridObjCutt.SetParam("mode", "search_Cutt");

	//WiseGrid가 서버와 통신시에 데이터를 전달한다.
	GridObjCutt.DoQuery(servlet_url);
}

/*신규등록*/
function doLineInsert() {
	var GridObjCutt = document.WiseGridCutt;
...
function doDelete(strColumnKey) {
	...
	var cnt = GridObjCutt.GetRowCount()
		chkCount = 0;
	...
	for(var i=0; i<cnt; i++) {
		...
		if(GridObjCutt.getCellValue("CRUD", i) != "NEW") {
			if(GridObjCutt.GetCellValue("SELECTED", i) == "1")
				GridObjCutt.DeleteRow(i);
		} else {
			if(GridObjCutt.GetCellValue("SELECTED", i) == "1")
				GridObjCutt.DeleteRow(i);
			i = i-1;
	...
	//GridObjCutt.DeleteRow(GridObjCutt.GetActiveRowIndex());
}
/*로우 체크 확인*/
function chkSelected() {
	chkCount = 0;

	for(i = 0; i < GridObjCutt.GetRowCount(); i++) { //그리드 데이터의 로우수를 반환한다.

		if(GridObjCutt.GetCellValue("SELECTED", i) == "1") //지정한 셀의 값을 가져온다.
			chkCount = chkCount + 1;
	}
...
function doSave() {
	...
	GridObjCutt.SetParam("mode", "save_Cutt");

	//WiseGrid가 서버와 통신시에 데이터를 전달한
	GridObjCutt.DoQuery(servlet_url, "CRUD");
}
...
function GridEndQuery() {
	...
	var mode = GridObjCutt.GetParam("mode");

		if(mode == "search_Cutt") {
		if(GridObjCutt.GetStatus() == "true") 	{// 서버에서 전송한 상태코드를 가져온다.
		} else	{
			var error_msg = GridObjCutt.GetMessage(); // 서버에서 전송한 상태코드값이 false라면 에러메세지를 가져온다.
			alert(error_msg);
		}
	}
...
<body>
<form name="cuttList">
<table width="974" height="293" border="0" cellpadding="0" cellspacing="0">
  <tr>
    <td height="31">절단비DataBase</td>
  </tr>
  <tr>
  ...
  <tr>
    <td><script>initWiseGrid("WiseGridCutt", "974", "400");</script></td>
  </tr>
</table>
</form>
</body>
