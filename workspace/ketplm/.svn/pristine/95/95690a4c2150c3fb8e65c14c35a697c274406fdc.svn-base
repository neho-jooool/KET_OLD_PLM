 ...
 *
 * <p>
 * Use the <code>newProductMaster</code> static factory method(s), not the
 * <code>ProductMaster</code> constructor, to construct instances of this
...
@GenAsPersistable(interfaces={OwnPersistable.class}, 
   ...
   properties={
   @GeneratedProperty(name="masterOid", type=String.class),
   },
   foreignKeys={
   @GeneratedForeignKey(myRoleIsRoleA=false,
      ...
      myRole=@MyRole(name="theProductMaster", cardinality=Cardinality.ONE)),
   @GeneratedForeignKey(myRoleIsRoleA=false,
      foreignKeyRole=@ForeignKeyRole(name="deliverCode", type=e3ps.common.code.NumberCode.class),
      myRole=@MyRole(name="theProductMaster", cardinality=Cardinality.ONE))
   })
public class ProductMaster extends _ProductMaster {
    ...
    **/
   public static ProductMaster newProductMaster()
            throws WTException {

      ProductMaster instance = new ProductMaster();
      instance.initialize();
      return instance;
   }
