 ...
 *
 * <p>
 * Use the <code>newKETWfmApprovalLine</code> static factory method(s),
 * not the <code>KETWfmApprovalLine</code> constructor, to construct instances
...
@GenAsPersistable(
   ...
   properties={
   @GeneratedProperty(name="approvalType", type=String.class, supportedAPI=SupportedAPI.PUBLIC,
      constraints=@PropertyConstraints(upperLimit=30)),
   @GeneratedProperty(name="approvalOrder", type=int.class, initialValue="0", supportedAPI=SupportedAPI.PUBLIC,
      constraints=@PropertyConstraints(upperLimit=2000)),
   @GeneratedProperty(name="approverID", type=String.class, supportedAPI=SupportedAPI.PUBLIC,
      constraints=@PropertyConstraints(upperLimit=30))
   },
   foreignKeys={
   @GeneratedForeignKey(name="KETWfmMasterLineLink", myRoleIsRoleA=false,
      foreignKeyRole=@ForeignKeyRole(name="appMaster", type=e3ps.wfm.entity.KETWfmApprovalMaster.class, supportedAPI=SupportedAPI.PUBLIC,
         constraints=@PropertyConstraints(required=true)),
      myRole=@MyRole(name="line", supportedAPI=SupportedAPI.PUBLIC))
   },
   tableProperties=@TableProperties(oracleTableSize=OracleTableSize.LARGE)
)
public class KETWfmApprovalLine extends _KETWfmApprovalLine {
    ...
    *
    * @return    KETWfmApprovalLine
    * @exception wt.util.WTException
    **/
   public static KETWfmApprovalLine newKETWfmApprovalLine()
            throws WTException {

      KETWfmApprovalLine instance = new KETWfmApprovalLine();
      instance.initialize();
      return instance;
   }
