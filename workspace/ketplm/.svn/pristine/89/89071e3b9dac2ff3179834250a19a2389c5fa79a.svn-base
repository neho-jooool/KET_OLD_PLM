...
<%
  ...
  headerFormat.setVerticalAlignment(jxl.format.VerticalAlignment.CENTRE);
    ...
    lr = new Label(4, row, "진행현황", headerFormat);
    s1.addCell(lr);

    lr = new Label(5, row, "TASK", headerFormat);
    s1.addCell(lr);

    lr = new Label(6, row, "주관부서", headerFormat);
    s1.addCell(lr);

    lr = new Label(7, row, "담당자", headerFormat);
    s1.addCell(lr);

    lr = new Label(8, row, "TASK완료", headerFormat);
    s1.addCell(lr);

    row++;

    lr = new Label(8, row, "계획일", headerFormat);
    s1.addCell(lr);

    lr = new Label(9, row, "실제일", headerFormat);
    s1.addCell(lr);

    lr = new Label(10, 2, "Issue 등록 여부", headerFormat);
    s1.addCell(lr);

    lr = new Label(11, 2, "TASK설명", headerFormat);
    s1.addCell(lr);

    lr = new Label(12, 2, "최상위 Task", headerFormat);
    s1.addCell(lr);
      ...
      if( list != null && list.size() > 0 ){
        for(int inx = 0 ; inx < list.size(); inx++){
            ...
            if( "5".equals(StringUtil.checkNull((String)project.get("taskstate"))) ){
                lr = new Label(4, row, "완료", cellFormat);
                s1.addCell(lr);
            }else{
                ...
                if(days < planend){
                    lr = new Label(4, row, "진행", cellFormat);
                    s1.addCell(lr);
                }else{
                    lr = new Label(4, row, "지연", cellFormat);
                    s1.addCell(lr);
                }
            }
            ...
            //TASK
            lr = new Label(5, row, StringUtil.checkNull((String)project.get("taskname")), cellFormat);
            s1.addCell(lr);

            //TASK 주관부서
            lr = new Label(6, row, StringUtil.checkNull((String)project.get("departName")), cellFormat);
            s1.addCell(lr);

            // TASK 담당자
            lr = new Label(7, row, StringUtil.checkNull((String)project.get("userName")), cellFormat);
            s1.addCell(lr);


            //완료일(계획)
            if (project.get("planenddate") != null && ((String)project.get("planenddate")).length() > 10){
              lr = new Label(8, row, ((String)project.get("planenddate")).substring(0, 10), cellFormat);
            }else{
                lr = new Label(8, row, StringUtil.checkNull((String)project.get("planenddate")), cellFormat);
            }
            s1.addCell(lr);

            //완료일
            if (project.get("execenddate") != null && ((String)project.get("execenddate")).length() > 10){
              lr = new Label(9, row, ((String)project.get("execenddate")).substring(0, 10), cellFormat);
            }else{
                lr = new Label(9, row, StringUtil.checkNull((String)project.get("execenddate")), cellFormat);
            }
            s1.addCell(lr);

            //issue 등록여부
            lr = new Label(10, row, StringUtil.checkNull((String)project.get("issue_yn")), cellFormat);
            s1.addCell(lr);

            // TASK 설명
            lr = new Label(11, row, StringUtil.checkNull((String)project.get("taskdesc")), cellFormat);
            s1.addCell(lr);

            //최상위 Task
            lr = new Label(12, row, StringUtil.checkNull(parentTaskName), cellFormat);
            s1.addCell(lr);
        }
    }
