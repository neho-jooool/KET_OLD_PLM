 ...
 *
 * <p>
 * Use the <code>newProjectDeptRole</code> static factory method(s), not
 * the <code>ProjectDeptRole</code> constructor, to construct instances
...
@GenAsPersistable(
   ...
   properties={
   @GeneratedProperty(name="deptRole", type=String.class,
      columnProperties=@ColumnProperties(index=true))
   },
   foreignKeys={
   @GeneratedForeignKey(myRoleIsRoleA=false,
      foreignKeyRole=@ForeignKeyRole(name="project", type=e3ps.project.TemplateProject.class,
         constraints=@PropertyConstraints(required=true)),
      myRole=@MyRole(name="theProjectDeptRole", cardinality=Cardinality.ONE)),
   @GeneratedForeignKey(myRoleIsRoleA=false,
      foreignKeyRole=@ForeignKeyRole(name="department", type=e3ps.groupware.company.Department.class,
         constraints=@PropertyConstraints(required=true)),
      myRole=@MyRole(name="theProjectDeptRole", cardinality=Cardinality.ONE))
   })
public class ProjectDeptRole extends _ProjectDeptRole {
    ...
    *
    * @return    ProjectDeptRole
    * @exception wt.util.WTException
    **/
   public static ProjectDeptRole newProjectDeptRole()
            throws WTException {

      ProjectDeptRole instance = new ProjectDeptRole();
      instance.initialize();
      return instance;
   }
