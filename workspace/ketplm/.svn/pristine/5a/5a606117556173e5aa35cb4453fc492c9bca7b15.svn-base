    ...
    public static void main( String[] args )
    {
        try
        {
            // Test encoding/decoding byte arrays
            {
                ...
                Kogger.debug(Base64.class,  "Bytes 2,2,3,0,9 as Base64: " + encodeBytes( bytes1 ) );
                Kogger.debug(Base64.class,  "Bytes 2,2,3,0,9 w/ offset: " + encodeBytes( bytes2, 1, bytes2.length-1 ) );
...
/* ********  D E C O D I N G   M E T H O D S  ******** */
    ...
    private static int decode4to3( byte[] source, int srcOffset, byte[] destination, int destOffset )
    {
        ...
        else
        {
            ...
            return 3;
                ...
                Kogger.debug(Base64.class, ""+source[srcOffset]+ ": " + ( DECODABET[ source[ srcOffset     ] ]  ) );
                Kogger.debug(Base64.class, ""+source[srcOffset+1]+  ": " + ( DECODABET[ source[ srcOffset + 1 ] ]  ) );
                Kogger.debug(Base64.class, ""+source[srcOffset+2]+  ": " + ( DECODABET[ source[ srcOffset + 2 ] ]  ) );
                Kogger.debug(Base64.class, ""+source[srcOffset+3]+  ": " + ( DECODABET[ source[ srcOffset + 3 ] ]  ) );
                return -1;
            }   //e nd catch
        }
    }   // end decodeToBytes
    ...
    public static byte[] decode( byte[] source, int off, int len )
    {
        ...
        for( i = 0; i < len; i++ )
        {
            ...
            {
                Kogger.error(Base64.class,  "Bad Base64 input character at " + i + ": " + source[i] + "(decimal)" );
                return null;
            }   // end else:
