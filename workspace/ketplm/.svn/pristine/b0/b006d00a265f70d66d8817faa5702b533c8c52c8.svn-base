var teamTaskProcessCompleteData = {
	
	/**
	 * server paging grid
	 */
	createPaingGrid : function(userName,startDate,endDate,departName){
		this.grid = TreeGrid(CommonGrid.getPagingGridConfig({
			id : 'SampleSearchGrid',
			Sort : '-taskName',
			perPageOnChange : 'javascript:teamTaskProcessCompleteData.search(Value);',
			Data : {
				Url : '/plm/ext/dashboard/teamTaskProcessCompleteDataList?sortName=*Sort0',
				Method : 'POST',
				Format : 'Form',
				Timeout : 0,
				Param : {
			...
			Head :[
					{
						...
						state : LocaleUtil.getMessage('01760'), stateRowSpan:'2', stateStateType : "Text",//'수정일'
		...
		//row click event
		Grids.OnClick = teamTaskProcessCompleteData.goView;
		
		//download page  event
		Grids.OnDownloadPage = function(grid,row){
			grid.Data.Page.Format = 'Form';
			grid.Data.Page.Method = 'Post';
			grid.Data.Page.Url = '/plm/ext/dashboard/teamTaskProcessCompleteDataList?sortName=*Sort0';
