var moldIssueProject = {
	
	/**
	 * server paging grid
	 */
	createPaingGrid : function(year,month,outsourcing,itemType,moldType,division){
		this.grid = TreeGrid(CommonGrid.getPagingGridConfig({
			id : 'SampleSearchGrid',
			//Sort : '-planStartDate',
			perPageOnChange : 'javascript:moldIssueProject.search(Value);',
			Data : {
				Url : '/plm/ext/dashboard/moldIssueProjectList?sortName=*Sort0',
				Method : 'POST',
				Format : 'Form',
				Timeout : 0,
				Param : {
			...
			Cols : [
					...
					{Name : 'createDate', 	Width:90,  Align : 'Center',Type:'Date', Format:"yyyy-MM-dd", CanSort : '1', CanEdit : '0' ,Spanned:'1'},
		...
		//row click event
		Grids.OnClick = moldIssueProject.goView;
		
		//download page  event
		Grids.OnDownloadPage = function(grid,row){
			grid.Data.Page.Format = 'Form';
			grid.Data.Page.Method = 'Post';
			grid.Data.Page.Url = '/plm/ext/dashboard/moldIssueProjectList?sortName=*Sort0';
