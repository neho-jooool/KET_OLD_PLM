$(document).ready(function(){
    ...
    //차종 suggest
    SuggestUtil.bind('CARTYPE', 'carTypeName','carType');
    //개발단계
    ...
    //개발단계 DEVYN으로 양산이 있는 것이라 process와는 혼동 불가
    //CommonUtil.singleSelect('occurStepCode',100);
    //<ket:select id="occurStepCode" name="occurStepCode" className="fm_jmp" style="width: 170px;" codeType="DEVYN" multiple="multiple" useCodeValue="true"/>
                    ...
                    for(var l=0; l < arrPInfosLength; l++) {
                            ...
                            */
                            ...
                            for(var p=0; p<processSize; p++){
                            	if(devYnOid == $("#process option:eq("+p+")").val()){
                            		...
                            		$("#process option").each(function(){
                            			$("#process option:eq("+p+")").attr('selected', false);
                            		});
                            	}
                            }
                        ...
                        // 고객처
                            ...
                            for(var s = 0; s < subcontractorOid.split(",").length; s++){
                            	$("select[name=customerDepartment]").append("<option value='"+subcontractorOid.split(",")[s]+"' selected>"+subcontractorName.split(",")[s]+"</option>");	
                            }
                        }
                        // 제품구분
