...
document.write('<script type="text/javascript" src="./js/res/' + strLangPath + '/res_upload_img_bg.js"></scrip' +'t>');
...
res_image[1] = "pop_upload_tab_image_1_r";
...
function fnInitResource()
{
	...
	for(var i=0; i<objSelRepeat.length; i++)
	{
		objSelRepeat.options[i].text = res_repeat_text[i];
	}
}
...
function fnFileInfoHTML(strFileName)
{
	try
	{
		...
		strFileInfoHTML += res_filename + "<br />";
