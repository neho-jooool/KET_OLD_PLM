 ...
 *
 * <p>
 * Use the <code>newProjectIssueAnswer</code> static factory method(s),
 * not the <code>ProjectIssueAnswer</code> constructor, to construct instances
...
@GenAsPersistable(interfaces={ContentHolder.class, OwnPersistable.class}, 
   ...
   properties={
   @GeneratedProperty(name="question", type=Object.class,
      columnProperties=@ColumnProperties(columnType=ColumnType.BLOB)),
   @GeneratedProperty(name="createDate", type=Timestamp.class),
   @GeneratedProperty(name="historyAttr", type=String.class),
   @GeneratedProperty(name="attr1", type=String.class),
   @GeneratedProperty(name="attr2", type=String.class),
   @GeneratedProperty(name="isCheck", type=boolean.class, initialValue="false"),
   @GeneratedProperty(name="planDate", type=Timestamp.class)
   },
   foreignKeys={
   @GeneratedForeignKey(myRoleIsRoleA=false,
      foreignKeyRole=@ForeignKeyRole(name="dependencyAnswer", type=e3ps.project.ProjectIssueAnswer.class,
         constraints=@PropertyConstraints(required=true)),
      myRole=@MyRole(name="theProjectIssueAnswer", cardinality=Cardinality.ONE))
   })
public class ProjectIssueAnswer extends _ProjectIssueAnswer {
    ...
    **/
   public static ProjectIssueAnswer newProjectIssueAnswer()
            throws WTException {

      ProjectIssueAnswer instance = new ProjectIssueAnswer();
      instance.initialize();
      return instance;
   }
