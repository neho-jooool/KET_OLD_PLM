...
<table border="0" cellspacing="0" cellpadding="0" width="780">
    <tr>
        <%if(type.trim().equals("normal")){%><td class="small" align="left">(<%=messageService.getString("e3ps.message.ket_message", "02499") %><%--전체페이지--%> : <%=totalPage%> )</td><%}%>
        <td>
        ...
        <%if(type.trim().equals("normal")){%><td class="small" align="right">(<%=messageService.getString("e3ps.message.ket_message", "02496") %><%--전체개수--%> : <%=totalCount%>)</td><%}%>
    </tr>
</table>
<!-- 페이지 설정 끝 //-->
