var assemblyNormal = {
	 ...
	 * server paging grid
	 */
	createPaingGrid : function(type, programNo){
		this.grid = TreeGrid(CommonGrid.getPagingGridConfig({
			id : 'SampleSearchGrid',
			Sort : '-pjtNo',
			perPageOnChange : 'javascript:assemblyNormal.search(Value);',
			Data : {
				Url : '/plm/ext/dashboard/assemblyNormalProjectList?sortName=*Sort0',
				Method : 'POST',
				Format : 'Form',
				Timeout : 0,
				Param : {
		...
		//row click event
		Grids.OnClick = assemblyNormal.goView;
		
		//download page  event
		Grids.OnDownloadPage = function(grid,row){
			grid.Data.Page.Format = 'Form';
			grid.Data.Page.Method = 'Post';
			grid.Data.Page.Url = '/plm/ext/dashboard/assemblyNormalProjectList?sortName=*Sort0';
