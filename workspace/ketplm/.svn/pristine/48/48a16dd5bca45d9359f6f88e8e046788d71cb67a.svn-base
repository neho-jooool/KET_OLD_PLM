...
public class IBAUtil
{
    ...
    {
        String returnStr = "";
        Class target = WTPart.class;
        if (ibaHolder instanceof EPMDocument) target = EPMDocument.class;
        ...
        select.appendWhere(new SearchCondition(target, "thePersistInfo.theObjectIdentifier.id", "=", CommonUtil
                .getOIDLongValue((Persistable) ibaHolder)), new int[] { idx });
        select.appendAnd();
    ...
    public static HashMap getAttributesHidNViewObject(IBAHolder ibaHolder) throws RemoteException, WTException, NumberFormatException,
            WTPropertyVetoException
    {
//Kogger.debug(getClass(), "ibaholder :: " + ibaHolder);
        IBAHolder iba = IBAValueHelper.service.refreshAttributeContainer(ibaHolder, null, null, null);
        DefaultAttributeContainer container = (DefaultAttributeContainer) iba.getAttributeContainer();
        HashMap hash = new HashMap();
        if (container == null) return hash;
            ...
            WTPropertyVetoException, WTException
    {
        ...
        Kogger.debug(IBAUtil.class, "IBAHolder == " + CommonUtil.getOIDString((Persistable)ibaHolder));
        ...
        Kogger.debug(IBAUtil.class, my.containsKey(hid));

        if (!my.containsKey(hid))
        {
        	Kogger.debug(IBAUtil.class, " 실행 된다 얍");
        ...
        else
        {
            ...
            if (obj instanceof StringValueDefaultView)
            {
                ...
                StringValue s = (StringValue) CommonUtil.getObject("" + sv.getObjectID());
                s.setValue(value);
            ...
            else if (obj instanceof FloatValueDefaultView)
            {
                ...
                FloatValue s = (FloatValue) CommonUtil.getObject("" + sv.getObjectID());
...
Kogger.debug(IBAUtil.class, "====setValuetoattributedata=========");
//WebUtil.print(ibaHash);
Kogger.debug(IBAUtil.class, "====setValuetoattributedata=========");
        Enumeration eu = ibaHash.keys();
        while (eu.hasMoreElements())
        {
            key = (String) eu.nextElement();
Kogger.debug(IBAUtil.class, "key >> " + key);
    ...
    public static DefaultAttributeContainer updateAttribute(IBAHolder ibaHolder, Hashtable hash)
    {
        ...
        try
        {
            Kogger.debug(IBAUtil.class, ">> updateAttributee :: " + ibaHolder);
            IBAHolder iba = IBAValueHelper.service.refreshAttributeContainer(ibaHolder, null, null, null);
 ...
 * Revision 1.1  2008/04/16 08:22:28  administrator
 * *** empty log message ***
 *
