 ...
 * 기간
 * <p>
 * Use the <code>newScheduleData</code> static factory method(s), not the
 * <code>ScheduleData</code> constructor, to construct instances of this
...
@GenAsPersistable(
   ...
   properties={
   @GeneratedProperty(name="duration", type=int.class,
      javaDoc="총 기간"),
   @GeneratedProperty(name="scheduleHistory", type=int.class,
      javaDoc="일정 이력≪≫≪≫0: 최초생성 시≪≫1~N: 갱신 시"),
   @GeneratedProperty(name="planStartDate", type=Timestamp.class),
   @GeneratedProperty(name="planEndDate", type=Timestamp.class,
      javaDoc="계획 종료일자"),
   @GeneratedProperty(name="stdWork", type=int.class,
      javaDoc="표준공수")
   })
public class ScheduleData extends _ScheduleData {
    ...
    **/
   public static ScheduleData newScheduleData()
            throws WTException {

      ScheduleData instance = new ScheduleData();
      instance.initialize();
      return instance;
   }
