...
<%
    ...
    epmVec = WorkflowSearchHelper.manager.getEPMList(multiReq);
    for(int idx=0; idx<epmVec.size(); idx++){
        EPMDocument epmDoc = (EPMDocument)epmVec.elementAt(idx);
        epmDoc = (EPMDocument)ObjectUtil.getLatestVersion(epmDoc);
        if(idx==0)epmOid = KETObjectUtil.getOidString(epmDoc);
        else epmOid = epmOid + "," + KETObjectUtil.getOidString(epmDoc);
    }
    epmOid.trim();
...
function save(){
    ...
    if(ok){
        ...
        document.createMulti.action = "/plm/servlet/e3ps/CommonWorkflowServlet?cmd=updateMulti&oid=<%=oid%>";
...
<input type="hidden" name="sendList" id="sendList" value="<%=epmOid %>">
          ...
          <td width="680" class="tdwhiteM0"><table border="0" cellspacing="0" cellpadding="0" width="680">
            ...
            <table width="670" cellpadding="0" cellspacing="0" class="table_border" name="epmList" id="epmList">
              <tr>
                ...
                <td width="120" class="tdgrayM"><%=messageService.getString("e3ps.message.ket_message", "01275") %><%--도면번호--%></td>
                <td width="190" class="tdgrayM"><%=messageService.getString("e3ps.message.ket_message", "01270") %><%--도면명--%></td>
              ...
              <%
              if(epmVec!=null){
                  for(int i=epmVec.size()-1; i>=0; i--){
                      EPMDocument epmDoc = (EPMDocument)epmVec.elementAt(i);
                      epmDoc = (EPMDocument)ObjectUtil.getLatestVersion(epmDoc);
                      String epmVersion = "";
                    String epmState = "";
                    boolean availableStateEPM = true;
                    if(!epmDoc.getState().getState().equals(State.toState("INWORK"))&&
                            !epmDoc.getState().getState().equals(State.toState("REWORK"))){availableStateEPM = false;}
                    if(KETObjectUtil.getVersion(epmDoc).equals("0")) epmVersion = KETObjectUtil.getVersion(epmDoc);
                    else epmVersion = "<font color=\"red\">"+KETObjectUtil.getVersion(epmDoc)+"</font>";
                    if(availableState && availableStateEPM)epmState = epmDoc.getState().getState().getDisplay(messageService.getLocale());
                    else epmState = "<font color=\"red\">"+epmDoc.getState().getState().getDisplay(messageService.getLocale())+"</font>";
              %>
              <tr>
                  <td class="tdwhiteM"><input type="checkbox" name="addcheck" id="addcheck" oid="<%=epmDoc.toString() %>"></td>
                  <td class="tdwhiteM"><%=i+1 %></td>
                  <td class="tdwhiteL" title="<%=epmDoc.getNumber() %>"><nobr style="text-overflow:ellipsis;overflow:hidden;width:120;"><%=epmDoc.getNumber() %></nobr></td>
                  <td class="tdwhiteL" title="<%=epmDoc.getName() %>"><nobr style="text-overflow:ellipsis;overflow:hidden;width:190;"><%=epmDoc.getName() %></nobr></td>
                  <td class="tdwhiteM"><%=epmDoc.getLifeCycleState().getDisplay(messageService.getLocale()) %></td>
