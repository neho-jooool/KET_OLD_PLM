...
<%@page import="ext.ket.part.util.PartUtil"%> 
...
<form name="innoditorDataForm2" method="post">
    ...
    <div class="contents-wrap" style="width:97%;">
        <div>
                ...
                <tbody>
                    ...
                    <tr>
                        <td class="border-bottom-none">
                            <div id="divView" style="overflow-x:auto; overflow-y:hidden; width: 350px"></div>
                        </td>
                        <td class="border-bottom-none">
                            <div id="divView2" style="overflow-x:auto; overflow-y:hidden; width: 350px"></div>
                        </td>
                    </tr>
                </tbody>
            </table>
			<table class="table-style-04" cellpadding="0">
                ...
                <tbody>
			        <tr>
			          ...
			          <td colspan="4" class="title"><%=messageService.getString("e3ps.message.ket_message", "04038") %><%--변경대상--%></td>
			        </tr>
				    <tr>
				      <td class="title"><%=messageService.getString("e3ps.message.ket_message", "04040") %><%--제품<BR>도면--%></td>    
				      <td class="title"><%=messageService.getString("e3ps.message.ket_message", "04041") %><%--승인<BR>도면--%></td>
				      <td class="title"><%=messageService.getString("e3ps.message.ket_message", "04042") %><%--고객<BR>제출<BR>도면--%></td>
				      <td class="title"><%=messageService.getString("e3ps.message.ket_message", "04043") %><%--ECAD<BR>도면--%></td>
				    </tr>
			        <%
                    ...
                    <%
                              /* 
                              Hashtable partInfo = ketBOMQueryBean.getPartInfo(bomHash.get("parent_item_code"));
                              WTPart wtPart = (WTPart) partInfo.get("WTPart");
                              */
                              String partOid = ketBOMQueryBean.getPartOid(bomHash.get("parent_item_code"), bomHash.get("eca_obj_after_rev"));
                              WTPart wtPart = (WTPart) CommonUtil.getObject(partOid);
                              
                              // 도면    
                              Map<String, List<EPMDocument>> epmDocumentMap = PartBaseHelper.service.getRelatedEPMDocByEcoPart(wtPart, false);
                              List<EPMDocument> epmPList = epmDocumentMap.get(RelatedEpmHandler.EPM_P);
                              List<EPMDocument> epmAList = epmDocumentMap.get(RelatedEpmHandler.EPM_A);
                              List<EPMDocument> epmCList = epmDocumentMap.get(RelatedEpmHandler.EPM_C);
                              List<EPMDocument> epmEList = epmDocumentMap.get(RelatedEpmHandler.EPM_E);
                              
                              String epmPListCheck = (epmPList != null && epmPList.size() > 0) ? "V" : "";
                              String epmAListCheck = (epmAList != null && epmAList.size() > 0) ? "V" : "";
                              String epmCListCheck = (epmCList != null && epmCList.size() > 0) ? "V" : "";
                              String epmEListCheck = (epmEList != null && epmEList.size() > 0) ? "V" : "";
                                                
                    %>          
                              <td align="center" class="border-bottom-none"><%=epmPListCheck %></td>
                              <td align="center" class="border-bottom-none"><%=epmAListCheck %></td>
                              <td align="center" class="border-bottom-none"><%=epmCListCheck %></td>
                              <td align="center" class="border-bottom-none"><%=epmEListCheck %></td>
                    ...
                    <%
                              Hashtable partInfo = ketBOMQueryBean.getPartInfo(bomHash.get("eca_obj_no"));
                              WTPart wtPart = (WTPart) partInfo.get("WTPart");
