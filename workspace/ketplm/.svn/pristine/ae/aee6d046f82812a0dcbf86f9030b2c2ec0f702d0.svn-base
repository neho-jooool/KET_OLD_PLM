var teamProjectTeamDelayList = {
	...
	createPaingGrid : function(departName,displayCode,startDate,endDate){
		this.grid = TreeGrid(CommonGrid.getPagingGridConfig({
			id : 'SampleSearchGrid',
			Sort : '-pjtName',
			perPageOnChange : 'javascript:teamProjectTeamDelayList.search(Value);',
			Data : {
				Url : '/plm/ext/dashboard/teamProjectTeamDelayListSetting?sortName=*Sort0',
				Method : 'POST',
				Format : 'Form',
				Timeout : 0,
				Param : {
		...
		//row click event
		Grids.OnClick = teamProjectTeamDelayList.goView;
		
		//download page  event
		Grids.OnDownloadPage = function(grid,row){
			grid.Data.Page.Format = 'Form';
			grid.Data.Page.Method = 'Post';
			grid.Data.Page.Url = '/plm/ext/dashboard/teamProjectTeamDelayListSetting?sortName=*Sort0';
