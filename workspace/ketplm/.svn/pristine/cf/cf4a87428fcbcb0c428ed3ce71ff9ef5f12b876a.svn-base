var carTaskTotal = {
	...
	createPaingGrid : function(pjtNo,carType,customer){
		this.grid = TreeGrid(CommonGrid.getPagingGridConfig({
			id : 'SampleSearchGrid',
			Sort : '-pjtNo',
			perPageOnChange : 'javascript:carTaskTotal.search(Value);',
			Data : {
				Url : '/plm/ext/dashboard/carTaskTotalSetting?sortName=*Sort0',
				Method : 'POST',
				Format : 'Form',
				Timeout : 0,
				Param : {
		...
		//row click event
		Grids.OnClick = carTaskTotal.goView;
		
		//download page  event
		Grids.OnDownloadPage = function(grid,row){
			grid.Data.Page.Format = 'Form';
			grid.Data.Page.Method = 'Post';
			grid.Data.Page.Url = '/plm/ext/dashboard/carTaskTotalSetting?sortName=*Sort0';
