var completeAndprocessProject = {
	...
	createPaingGrid : function(pjtType,type,devType,year){
		this.grid = TreeGrid(CommonGrid.getPagingGridConfig({
			id : 'SampleSearchGrid',
			Sort : '-pjtNo',
			perPageOnChange : 'javascript:completeAndprocessProject.search(Value);',
			Data : {
				Url : '/plm/ext/dashboard/completeAndprocessProjectList1?sortName=*Sort0',
				Method : 'POST',
				Format : 'Form',
				Timeout : 0,
				Param : {
		...
		//row click event
		Grids.OnClick = completeAndprocessProject.goView;
		
		//download page  event
		Grids.OnDownloadPage = function(grid,row){
			grid.Data.Page.Format = 'Form';
			grid.Data.Page.Method = 'Post';
			grid.Data.Page.Url = '/plm/ext/dashboard/completeAndprocessProjectList1?sortName=*Sort0';
