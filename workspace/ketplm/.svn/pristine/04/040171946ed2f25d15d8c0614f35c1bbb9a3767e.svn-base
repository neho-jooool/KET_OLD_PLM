...
function popupProject(targetId) {
  ...
  var selectedAry = window.showModalDialog(url, window,"help=no; resizable=no; status=no; scroll=yes; dialogWidth=1024px; dialogHeight:768px; center:yes");
  if (selectedAry != undefined && selectedAry.length > 0) {
    var isAppend = "Y";
    setProject(targetId, selectedAry, isAppend);
  }
}
...
$(document).ready(function(){
  ...
  $('[name=create_start]').val('<%=predate%>');
  $('[name=create_end]').val('<%=postdate%>');
  
  // Project 구분 radio click event
  $("[name=searchPjtType]").click(function() {
      CommonUtil.deleteValue('projectNo','projectNo');
      if("" == $(this).val()){
          $("[name=projectNo]").unbind();
      } else if("제품" == $(this).val()){
          SuggestUtil.bind('PRODUCTPROJNO', 'projectNo');
      } else if("검토" == $(this).val()){
          SuggestUtil.bind('REVIEWPROJNO', 'projectNo');
      } else if("금형" == $(this).val()){
          SuggestUtil.bind('DIENO', 'projectNo');
      }
  });
  ...
  //search();
    ...
    setProjectNo = function(returnValue){
        ...
        $('[name=projectNo]').val(proj_number);
    }
    
    function selProjectNo() {
    	SearchUtil.selectOneProjectPopUp('setProjectNo');
    }
...
<body class="body-space" >
  <form name="form01" autocomplete="off">
    ...
    <%-- <input type="hidden" name="businessType" value="<%= CommonUtil.getDivisionNumberCodeOidFromSessionUser() %>" /> --%>
    <input type="hidden" name="businessType" value="" />
    <input type="hidden" id="creator" name="creator" value="<%= CommonUtil.getOIDString(SessionHelper.manager.getPrincipal()) %>">
    <!-- hidden end -->
        ...
        <td valign="top">
              ...
              <td>
                <table style="width: 100%; height: 28px;" border="0" cellspacing="0" cellpadding="0">
                  <tr>
                    ...
                    <td class="font_01">My Drawing</td>
                    <td align="right"><img src="/plm/portal/images/icon_navi.gif">Home<img src="/plm/portal/images/icon_navi.gif"><%=messageService.getString("e3ps.message.ket_message", "02435")%><%--작업공간--%>
                      <img src="/plm/portal/images/icon_navi.gif">My Drawing</td>
                  </tr>
                </table>
              </td>
            </tr>
          ...
          <table border="0" cellspacing="0" cellpadding="0" style="width: 100%;">
            <tr>
              ...
              <td style="width: 270px;" class="tdwhiteL"><input type="hidden" id="partNumberMasterOid" name="partNumberMasterOid" value="<%=searchCondition.getString("partNumberMasterOid")%>">
                <input type="text" id="partNumber" name="partNumber" class="txt_field" style="width: 80%" value="<%=partNumber%>"> <a href="javascript:;"
                onClick="javascript:selectPartNo('partNumber', 'm');"
              ><img src="/plm/portal/images/icon_5.png" border="0"></a> &nbsp;<a href="javascript:;" onClick="javascript:clearPartNo('partNumber');"><img
              ...
              <td class="tdwhiteL0"><select id="state" name="state" class="fm_jmp" style="width: 270px" multiple="multiple">
                  <%
            ...
            <tr>
              ...
              <td style="width: 270px;" class="tdwhiteL"><input type="text" name="number" class="txt_field" style="width: 98%" value="<%=number%>" id="number"></td>
              <td style="width: 110px;" class="tdblueL"><%=messageService.getString("e3ps.message.ket_message", "01270")%><%--도면명--%></td>
              <td style="width: 270px;" class="tdwhiteL0"><input type="text" name="name" class="txt_field" style="width: 98%" value="<%=name%>" id="name"></td>
            </tr>
            <tr>
              <td class="tdblueL"><%=messageService.getString("e3ps.message.ket_message", "01264")%><%--도면구분--%></td>
              <td class="tdwhiteL"><select id="devStage" name="devStage" class="fm_jmp" style="width: 270px" onFocus='this.initialSelect = this.selectedIndex;' multiple="multiple">
              ...
              <td class="tdwhiteL0"><select id="category" name="category" class="fm_jmp" style="width: 270px" onFocus='this.initialSelect = this.selectedIndex;' multiple="multiple">
                  <!-- onChange='this.selectedIndex = this.initialSelect;'>  -->
                  <%
                  ...
                  %>
                  <OPTION VALUE="">-----<%=cmt.getDisplay(messageService.getLocale())%>-----
                  </OPTION>
                  <%
            ...
            <tr>
              ...
              <td class="tdwhiteL"><select id="cadAppType" name="cadAppType" class="fm_jmp" style="width: 270px" onFocus='this.initialSelect = this.selectedIndex;' multiple="multiple">
                  <!-- onChange='this.selectedIndex = this.initialSelect;'>  -->
                  <%
              ...
              <td class="tdwhiteL0"><input name="latest" type="radio" class="Checkbox" value="true" <%if ("true".equals(latest)) {%> checked <%}%>><%=messageService.getString("e3ps.message.ket_message", "02839")%><%--최신--%>
                <input name="latest" type="radio" class="Checkbox" value="false" <%if ("false".equals(latest)) {%> checked <%}%>><%=messageService.getString("e3ps.message.ket_message", "02485")%><%--전체--%>
              </td>
            </tr>
            <tr>
            </tr>
            <tr>
              ...
              <td class="tdwhiteL0" colspan="3"><input type="radio" name="searchPjtType" value="" checked="checked"><%=messageService.getString("e3ps.message.ket_message", "02485")%><%--전체--%>
                <input type="radio" name="searchPjtType" value="제품"><%=messageService.getString("e3ps.message.ket_message", "02536")%><%--제품--%> <input type="radio" name="searchPjtType"
                  value="금형"
                ><%=messageService.getString("e3ps.message.ket_message", "00997")%><%--금형--%> <input type="radio" name="searchPjtType" value="검토"><%=messageService.getString("e3ps.message.ket_message", "00716")%><%--검토--%>
                <input type="text" id="projectNo" name="projectNo" class="txt_field" style="width: 200px;" value="<%=projectNo%>"> <a href="javascript:selProjectNo()"><img
                  src="/plm/portal/images/icon_5.png" border="0"
                ></a> <a href="javascript:CommonUtil.deleteValue('projectNo','projectNo');"> <img src="/plm/portal/images/icon_delete.gif" border="0"></a></td>
            </tr>
            <tr>
              <td class="tdblueL"><%=messageService.getString("e3ps.message.ket_message", "02429")%><%--작성일자--%></td>
              <td class="tdwhiteL0" colspan="3"><input type="text" id="create_start" name="create_start" class="txt_field" style="cursor: hand;" value=""> &nbsp;<img
                src="/plm/portal/images/icon_delete.gif" border="0" onclick="javascript:CommonUtil.deleteDateValue('create_start');"
              > &nbsp;~&nbsp; <input type="text" id="create_end" name="create_end" class="txt_field" style="cursor: hand;" value=""> &nbsp;<img src="/plm/portal/images/icon_delete.gif"
                border="0" onclick="javascript:CommonUtil.deleteDateValue('create_end');"
              ></td>
            </tr>
          </table>
          <table border="0" cellspacing="0" cellpadding="0" style="width: 100%;">
              ...
              <div class="row-fluid">
                <div id="listGrid" style="WIDTH: 100%; HEIGHT: 100%">
                  <bdo <%--
                    ...
                    Layout_Url="/plm/jsp/edm/SearchEPMDocumentGridLayout.jsp" Layout_Param_Pagingsize="<%=pagingSize%>" Data_Url="/plm/servlet/e3ps/EDMServlet?command=searchGridData&businessType=<%= CommonUtil.getDivisionNumberCodeOidFromSessionUser() %>&creator=<%= CommonUtil.getOIDString(SessionHelper.manager.getPrincipal()) %>&create_start=<%=predate %>&create_end=<%=postdate %>" Data_Format="Internal"
                    Data_Data="TGData" Data_Method="POST" Page_Url="/plm/servlet/e3ps/EDMServlet?command=searchGridPage&businessType=<%= CommonUtil.getDivisionNumberCodeOidFromSessionUser() %>&creator=<%= CommonUtil.getOIDString(SessionHelper.manager.getPrincipal()) %>&create_start=<%=predate %>&create_end=<%=postdate %>" Page_Format="Internal" Page_Data="TGData" Page_Method="POST"
