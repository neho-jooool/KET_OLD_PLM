var CreateProgram = {
	...
	createEventGrid : function(){
		this.grid = TreeGrid(CommonGrid.getGridConfig({
			...
			Cols : [
				...
				{Name : 'lastUsingBuyerName', Width:150, Align : 'Center', CanSort : '0', CanEdit : '0', Visible : '0'},
				{Name : 'lastUsingBuyerOid', Width:150, Align : 'Center', CanSort : '0', CanEdit : '0', Visible : '0'}
			],Header :{
	...
	createProjectLinkGrid : function(isEditable){
		...
		Grids.OnRenderFinish = function(G){
        	...
        	CalendarUtil.dateInputFormat('customerEventDate',CreateProgram.checkDateValidation,{
...
//		}
	...
	validation : function(){
		...
		for ( var tRow = G.GetFirstVisible(); tRow; tRow = G.GetNextVisible(tRow) ) {
			...
			eventDates.push(tRow.carEventDate);
			
			i++;
	    }
		
		var param = new Object();
		param.oid = $("input[name=carType]").val();
		var optData = ajaxCallServer("/plm/ext/common/getObjectData", param, null, false);
		
		if(!eval(optData.isRunningChange)){
			
			var isDateError = false;
			
			$('input[name=customerEventDate]').each(function(i){
				
				var target = checkNull($(this).val());
				var source = checkNull(eventDates[i]);
				
				var targetDate = new Date(target);
				var sourceDate = new Date(source);
				
				if(!isDateError && targetDate.getTime() > sourceDate.getTime()){
					 isDateError = true;
				 }
			});

			if(isDateError){
				alert('고객 일정이 차종 일정보다 이후 인 건이 존재합니다.\nPM, 영업 담당자에게 문의하여 일정 확인 후 진행하시기 바랍니다.');
				return false;
			}
		}
	...
	setPrefixName : function(){
		...
		$('[name=lastUsingBuyerTxt]').val(Grids[0].GetString(Grids[0].GetFirstVisible(),'lastUsingBuyerName'));
		$('[name=lastUsingBuyer]').val(Grids[0].GetString(Grids[0].GetFirstVisible(),'lastUsingBuyerOid'));
