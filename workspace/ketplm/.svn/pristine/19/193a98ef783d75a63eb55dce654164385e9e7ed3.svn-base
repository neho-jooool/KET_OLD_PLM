...
public class EDMMigDataLoader implements wt.method.RemoteAccess, java.io.Serializable {
	...
	try {
	    ...
	    Kogger.error(EDMMigDataLoader.class, e);
	}
    }
	...
	try {
	    ...
	    if (!dataFile.exists()) {
		Kogger.debug(EDMMigDataLoader.class, "File not found!!!");
		return false;
	    }
	    ...
	    while (itr.hasNext()) {
		Kogger.debug(EDMMigDataLoader.class, ">" + (String) itr.next());
	    }
		...
		for (int i = 0; i < sheets.length; i++) {
			...
			Cell hdr = hdrs[k];
			Kogger.debug(EDMMigDataLoader.class, "Column name : " + hdr.getContents());
		    }

		    for (int k = 1; k < rows; k++) {
			//Kogger.debug(getClass(), "current row : " + i);
			//Cell cell[] = sheet.getRow(k);
			EDMMigData rdata = (EDMMigData) getRowData(sheet.getRow(k), k + 1, sheet.getName());
			...
			if (rdata != null) {
				...
				for (int m = 0; m < v0.size(); m++) {
				    EDMMigData rdata0 = (EDMMigData) v0.get(m);
	...
	try {
	    ...
	    else {	// 2011.05.02 고객제출도면 체크 (도면번호, 부품번호, 파일명이 다를때 Migration 실패  by TaeHun
		if ("CU".equals(data.number.substring(0, 2))) {
		    ...
		    Kogger.debug(EDMMigDataLoader.class, "CU ::: " + data.number.substring(0, 2));
		    Kogger.debug(EDMMigDataLoader.class, "도면번호 ::: " + subNumer);
		    Kogger.debug(EDMMigDataLoader.class, "파일명 ::: " + subRepPartNumer);
		    Kogger.debug(EDMMigDataLoader.class, "부품번호 ::: " + subCadFileNumer);
	...
	if (!SERVER) {
	    ...
	    Object obj = null;
	    try {
		obj = wt.method.RemoteMethodServer.getDefault().invoke("save", EDMMigDataLoader.class.getName(), null, argTypes, args);
	...
	try {
		...
		EDMMigData emd = (EDMMigData) data.get(i);
		...
		if (IS_CREATE_PART) {
			...
			Kogger.debug(EDMMigDataLoader.class, "create part : " + part.getNumber());
		    }
		}
    ...
    public static EDMMigData save(EDMMigData data) throws WTException {
	try {
	    if (!data.isValidate) {
		return data;
	    }
	    ...
	    Folder folder = FolderTaskLogic.getFolder(data.getLocation(), wtContainerRef);
    ...
    public static EDMMigData update(EDMMigData data) throws WTException {
	try {
	    if (!data.isValidate) {
		return data;
	    }
	...
	Kogger.debug(EDMMigDataLoader.class, sb.toString());
    }
	...
	Kogger.debug(EDMMigDataLoader.class, "Load Start...");
	...
	Kogger.debug(EDMMigDataLoader.class, "Load End...");

	long endTimeMillis = System.currentTimeMillis();
	Kogger.debug(EDMMigDataLoader.class, "Load Time : " + (endTimeMillis - startTimeMillis));
	System.exit(0);
    }
}

class EDMMigData implements Serializable {
    ...
    public EDMMigData(int rownum) {
	this.rownum = rownum;
    }

    public EDMMigData(Cell[] cells, int rownum) {
	this.rownum = rownum;
	setCellData(cells);
    }
	...
	if("PRODUCTION_STAGE".equals(devStage)) {
		im.put(EDMHelper.IBA_MANUFACTURING_VERSION, "P"+ cadVersion);
	}
	*/
	return im;
    }
