...
public class KETExcelUtil {
	...
	public String createExcelFile(ArrayList keyList, ArrayList dataList, String pageName) throws IOException {
				...
				} else // Data
				{
					...
					for (int j = 0; j < keyList.size(); j++) {
						...
						if (!tmp.equals("") && tmp.indexOf("<font color=\"#FF0000\">") != -1) { // RED
							...
							tmp = tmp.replace("<font color=\"#FF0000\"><B>", "");
							tmp = tmp.replace("</B></font>", "");
							...
							tmp = tmp.replace("<font color=\"#0000FF\"><B>", "");
							tmp = tmp.replace("</B></font>", "");

						} else if (!tmp.equals("") && tmp.indexOf("<font color=\"#34862E\">") != -1) { // GREEN
							...
							tmp = tmp.replace("<font color=\"#34862E\"><B>", "");
							tmp = tmp.replace("</B></font>", "");
		...
		HSSFRow cheeckRow = sheet.getRow(0);
		for (int k = 0; k < keyList.size(); k++) {
			sheet.autoSizeColumn((k), false);
			sheet.setColumnWidth(k, (sheet.getColumnWidth(k) + 512));

			HSSFCell checkCell = cheeckRow.getCell(k);

			String headerNm = checkCell.getStringCellValue();
			Kogger.debug(getClass(), "checkCell.getStringCellValue(" + k + ")===>" + headerNm);
			if (headerNm.equals("ECO No")) {
				sheet.setColumnWidth(k, (sheet.getColumnWidth(k) + 2560));
			}
	...
	public static void main(String[] args) throws IOException {
		...
		Kogger.debug(KETExcelUtil.class, designDate);
	}
}
