var projectCardIssue = {
	...
	createPaingGrid : function(type,pjtNo){
		this.grid = TreeGrid(CommonGrid.getPagingGridConfig({
			id : 'SampleSearchGrid',
			Sort : '-createDate',
			perPageOnChange : 'javascript:projectCardIssue.search(Value);',
			Data : {
				Url : '/plm/ext/dashboard/projectCardIssueList?sortName=*Sort0',
				Method : 'POST',
				Format : 'Form',
				Timeout : 0,
				Param : {
		...
		//row click event
		Grids.OnClick = projectCardIssue.goView;
		
		//download page  event
		Grids.OnDownloadPage = function(grid,row){
			grid.Data.Page.Format = 'Form';
			grid.Data.Page.Method = 'Post';
			grid.Data.Page.Url = '/plm/ext/dashboard/projectCardIssueList?sortName=*Sort0';
