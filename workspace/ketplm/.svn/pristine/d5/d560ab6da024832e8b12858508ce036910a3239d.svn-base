var teamTaskProcessTeamPlanData = {
	...
	createPaingGrid : function(displayCode,startDate,endDate,departName){
		this.grid = TreeGrid(CommonGrid.getPagingGridConfig({
			id : 'SampleSearchGrid',
			Sort : '-taskName',
			perPageOnChange : 'javascript:teamTaskProcessTeamPlanData.search(Value);',
			Data : {
				Url : '/plm/ext/dashboard/teamTaskProcessTeamPlanDataList?sortName=*Sort0',
				Method : 'POST',
				Format : 'Form',
				Timeout : 0,
				Param : {
		...
		//row click event
		Grids.OnClick = teamTaskProcessTeamPlanData.goView;
		
		//download page  event
		Grids.OnDownloadPage = function(grid,row){
			grid.Data.Page.Format = 'Form';
			grid.Data.Page.Method = 'Post';
			grid.Data.Page.Url = '/plm/ext/dashboard/teamTaskProcessTeamPlanDataList?sortName=*Sort0';
