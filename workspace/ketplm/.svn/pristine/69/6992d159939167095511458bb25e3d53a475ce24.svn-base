...
public class ZipUtil {
	...
	try {
		...
		if (zentry.isDirectory()) {// Directory 인 경우
		    FileUtil.checkDir(targetFile.getAbsolutePath()); // 디렉토리 생성
		} else { // File 인 경우
			 // parent Directory 생성
		    FileUtil.checkDir(targetFile.getParent());
    ...
    protected static void copyFilesByExtInDir(String sourcePath, String suffix, String outputPath) throws Exception {
	File sourceFile = new File(sourcePath);
	File[] fileArray = sourceFile.listFiles(); // sourceFile 의 하위 파일 리스트
	for (int i = 0; i < fileArray.length; i++) {
	    File file = fileArray[i];
	    if (file.getName().endsWith(suffix)) {
		FileUtil.CopyServerToServer(file.getAbsolutePath(), outputPath + (outputPath.endsWith(File.separator) ? "" : File.separator) + file.getName());
	    }
	}
    }

    public static void main(String[] args) throws Exception {
	// String szOriginFileName = "D:\\bandsi\\workspace\\.metadata\\version.ini";
	// String szCopyFileName = "D:\\temp\\version.ini";
	// FileUtil.CopyServerToServer(szOriginFileName, szCopyFileName);
	String sourcePath = "D:\\ket\\workspace\\ketplm\\src\\e3ps\\bom";
	String outputPath = "D:\\temp";
	String suffix = "l.java";

	copyFilesByExtInDir(sourcePath, suffix, outputPath);

	String sourcePath1 = "D:\\temp";
	String output = "D:\\temp.zip";

	ZipUtil.zip(sourcePath1, output);
    }
}
