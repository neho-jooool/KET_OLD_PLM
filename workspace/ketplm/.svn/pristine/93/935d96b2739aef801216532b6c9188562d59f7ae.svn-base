...
<%@page import="e3ps.edm.util.VersionHelper" %>
...
// JSP 디버깅용 FLAG : 부품검색시 체크박스 안보일 때 디버깅 등에서 사용
boolean enableDebug = false;
if(enableDebug){ Kogger.debug("####$$$$ SearchActivityBomSearchResultForm.jsp : debugging " ); }
...
if(data.getTotalCount() > 0) {
    ...
    for(int i=0; i < size; i++) {
        ...
        if(i == 0) {
            pNumsParameter = StringUtil.checkNull(ketSearchEcoDetailVO.getPartNumber());
        } else {
            pNumsParameter = ","+ StringUtil.checkNull(ketSearchEcoDetailVO.getPartNumber());
        }
    }
...
String[] pNums = StringUtils.splitPreserveAllTokens(pNumsParameter, ",");
*/
...
<div style="position:; width:100%; height:405px; overflow-x:auto; overflow-y:auto;">
...
<table border="0" cellspacing="0" cellpadding="0" width="100%" class="table_border" id="listBomChildTable">
  <tr>
    ...
    <td width="200" rowspan="3" class="tdblueM"><%=messageService.getString("e3ps.message.ket_message", "01569") %><%--부품번호--%></td>
	...
	*/
	...
	if(data.getTotalCount() > 0) {
	    ...
	    for(int i=0; i < size; i++) {
		    ...
		    // 부품 OID
		    String oid = ketSearchEcoDetailVO.getOid();
	        /*     
	        // 제품 품번
	        String productCode = ketSearchEcoDetailVO.getPartNumber();
	        // 부품번호
	        String partNo = ketSearchEcoDetailVO.getPartNumber();
	        // 부품유형
	        String partType1 = ketSearchEcoDetailVO.getPartType();
	        // 부품명
	        String partName = ketSearchEcoDetailVO.getPartName();
	            
	        // 리비전
	        String rev = ketSearchEcoDetailVO.getPartVersion();
	        // Display 리비전
	        String revDis = ketSearchEcoDetailVO.getPartVersion();
	            
	        // 상태
	        String state = ketSearchEcoDetailVO.getSancStateFlag();
	        // 모부품번호
	        String parentNo = ketSearchEcoDetailVO.getPartNumber();
	        */
	             
		    wtPart = (WTPart) CommonUtil.getObject(oid);
	        
		    // 최신 리비전
		    Versioned versioned = (Versioned) wtPart;
		    WTPart latestWTPart = (WTPart) VersionHelper.getLatestRevision(versioned);
		    
		    // 제품 품번
            String productCode = latestWTPart.getNumber();
            // 부품번호
            String partNo = latestWTPart.getNumber();
            if(enableDebug){ Kogger.debug("####$$$$ partNo :" + partNo ); }
            
            // 부품유형
            String partType1 = ketSearchEcoDetailVO.getPartTypeName();  // PartSpecGetter.getPartSpec(latestWTPart, PartSpecEnum.SpPartType);
            // 부품명
            String partName = latestWTPart.getName();
                
            // 리비전
            String rev = latestWTPart.getVersionInfo().getIdentifier().getValue();
            // Display 리비전
            String revDis = StringUtils.stripToEmpty(PartSpecGetter.getPartSpec(latestWTPart, PartSpecEnum.SpPartRevision));
            
            // 부품 초도여부
            String bomFlag = StringUtils.stripToEmpty(PartSpecGetter.getPartSpec(latestWTPart, PartSpecEnum.SpBOMFlag));
            
            // 상태
            String state = latestWTPart.getLifeCycleState().getDisplay();
            // 모부품번호
            String parentNo = "";
		    ...
		    //if (ext.ket.part.util.PartUtil.isProductType(part.get("typeCode"))) {
		        ...
		        die_no = PartSpecGetter.getPartSpec(latestWTPart, PartSpecEnum.SpRepresentM);
		    ...
		    */
            ...
            if((prodMoldCls != null && prodMoldCls.equals("2")) && PartUtil.isProductType(latestWTPart)) {
        	    if(enableDebug){ Kogger.debug("####$$$$ isAbled = false : case 1"); }
                isAbled = false;
            } 
            // 제품에서 호출하고 부품이 금형(일반제품이 아닐)일 경우
            else if((prodMoldCls == null || prodMoldCls.equals("1")) && !PartUtil.isProductType(latestWTPart)) {
                isAbled = false;
            }
            ...
            if(isAbled) {
                ...
                if(currentEcoNo == null || currentEcoNo.equals("") || currentEcoNo.equalsIgnoreCase("null")) {
                    if(!ecoIds.equals("")) {
                	    if(enableDebug){ Kogger.debug("####$$$$ isAbled = false : case 2"); }
                        isAbled = false;
                    } else {
                        ...
                        if(isTheFirst) {
                            ...
                            if(!rev.equalsIgnoreCase("0") || !state.equals("작업중")) {
                        	    if(enableDebug){ Kogger.debug("####$$$$ isAbled = false : case 3"); }
                                isAbled = false;
                            }
                        }
                        // 설변일 경우
                            ...
                            */      
                            if(isAbled) {
                                ...
                                if(rev.equalsIgnoreCase("0") && state.equals("작업중")) {
                                    if(enableDebug){ Kogger.debug("####$$$$ isAbled = true : case -1"); }
                                    isAbled = true;
                                } else {
                                    // 승인완료가 아니면 선택불가
                                    if(!state.equals("승인완료")) {
                                	    if(enableDebug){ Kogger.debug("####$$$$ isAbled = false : case 4"); }
                                        isAbled = false;
                                    }
                    ...
                    if(!ecoIds.equals("") && ecoIds.lastIndexOf(currentEcoNo) > -1) {
                	   if(enableDebug){ Kogger.debug("####$$$$ isAbled = true : case -2"); }
                	    isAbled = true;
                    } else {
                        
                	    if(!ecoIds.equals("")) {
                		    if(enableDebug){ Kogger.debug("####$$$$ isAbled = false : case 5"); }
                		    isAbled = false;
                	    } else {
	                        // 초도일 경우 
	                        if(isTheFirst) {
	                            // 작업중이 아니면 선택불가
	                            if(!rev.equalsIgnoreCase("0") || !state.equals("작업중")) {
                                    if(enableDebug){ Kogger.debug("####$$$$ isAbled = false : case 6"); }      
	                                isAbled = false;
	                            }
	                        }
	                        // 설변일 경우
	                            ...
	                            */   
	                            if(isAbled) {
	                                ...
	                                // 부품이든 도면이든 OOTB 리비전이 '0'이면서 작업중인 것은 첨부가능
	                                if(rev.equalsIgnoreCase("0") && state.equals("작업중")) {
	                                    if(enableDebug){ Kogger.debug("####$$$$ isAbled = true : case -3"); }
                                        isAbled = true;
	                                } else {
	                                    // 승인완료가 아니면 선택불가
	                                    if(!state.equals("승인완료")) {
                                            if(enableDebug){ Kogger.debug("####$$$$ isAbled = false : case 7"); }      
	                                        isAbled = false;
	                                    }
            ...
            if(enableDebug){ Kogger.debug("####$$$$ isAbled last :" + isAbled); }
	        ...
	        if(enableDebug){ Kogger.debug("####$$$$ epmPListSize size :" + epmPListSize); }
	        if(enableDebug){ Kogger.debug("####$$$$ epmAListSize size :" + epmAListSize); }
	        if(enableDebug){ Kogger.debug("####$$$$ epmCListSize size :" + epmCListSize); }
	        if(enableDebug){ Kogger.debug("####$$$$ epmEListSize size :" + epmEListSize); }
	        
    %>          
		<tr>

		  <td class="tdwhiteM"><%=(i+1) %></td>
          <td class="tdwhiteM" title="<%=StringEscapeUtils.escapeHtml(partNo) %>"><%=partNo %></td>
		  <td class="tdwhiteM"><%=partType1 %></td>
          <td class="tdwhiteL" title="<%=StringEscapeUtils.escapeHtml(partName) %>"><div class="ellipsis" style="width:200px;"><nobr><%=partName %></nobr></div></td>
