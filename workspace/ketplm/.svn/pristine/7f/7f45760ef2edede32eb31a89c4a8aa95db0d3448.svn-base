...
public class UpgradeBoardServlet extends ManageHelpBoardServlet {
	...
	String cause = map.getString("cause");
	String handleResult = map.getString("handleResult");
	String managerOid = map.getString("managerOid");
	...
	if (StringUtil.checkString(cause)) {
	    newBoard.setCause(cause);
	}

	if (StringUtil.checkString(handleResult)) {
	    newBoard.setHandleResult(handleResult);
	}
	...
	if (StringUtil.isEmpty(poid) == false && HelpBoardUtil.isValidOid(poid)) {
		...
		if("SYSERR".equals(division)){
		    newBoard.setUser(admin);
		}else{
		    if(StringUtils.isNotEmpty(managerOid)){
			newBoard.setUser((WTUser)CommonUtil.getObject(managerOid));
		    }else{
			newBoard.setUserReference(SessionHelper.manager.getPrincipalReference());
		    }
		}
	    }
	}
	...
	if (StringUtil.checkString(cause)) {
	    board.setCause(cause);
	}

	if (StringUtil.checkString(handleResult)) {
	    board.setHandleResult(handleResult);
	}
	...
	if (StringUtil.isEmpty(poid) == false && HelpBoardUtil.isValidOid(poid)) {
		...
		if("SYSERR".equals(division)){
		    ...
		    if(StringUtils.isNotEmpty(managerOid)){
			board.setUser((WTUser)CommonUtil.getObject(managerOid));
		    }else{
			board.setUserReference(SessionHelper.manager.getPrincipalReference());
		    }
		}
