var teamQulityList = {
	 ...
	 * server paging grid
	 */
	createPaingGrid : function(userName, departName,startDate,endDate){
		this.grid = TreeGrid(CommonGrid.getPagingGridConfig({
			id : 'SampleSearchGrid',
			Sort : '-pjtNo',
			perPageOnChange : 'javascript:teamQulityList.search(Value);',
			Data : {
				Url : '/plm/ext/dashboard/teamQulityListSetting?sortName=*Sort0',
				Method : 'POST',
				Format : 'Form',
				Timeout : 0,
				Param : {
		...
		//row click event
		Grids.OnClick = teamQulityList.goView;
		
		//download page  event
		Grids.OnDownloadPage = function(grid,row){
			grid.Data.Page.Format = 'Form';
			grid.Data.Page.Method = 'Post';
			grid.Data.Page.Url = '/plm/ext/dashboard/teamQulityListSetting?sortName=*Sort0';
