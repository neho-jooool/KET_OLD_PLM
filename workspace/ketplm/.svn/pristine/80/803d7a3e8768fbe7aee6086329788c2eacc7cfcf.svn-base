...
<script language="JavaScript" for="WiseGridReer" event="Initialize()">
initReer();
</script>
<!--  	서버와의 통신이 정상적으로 완료되면 발생한다.   -->
<script language=javascript for="WiseGridReer" event="EndQuery()">
	GridEndQuery();
</script>
...
function doQuery(){
	...
	GridObjReer.SetParam("mode", "search_Reer");

	//WiseGrid가 서버와 통신시에 데이터를 전달한다.
	GridObjReer.DoQuery(servlet_url);
}

/*신규등록*/
function doLineInsert() {
	var GridObjReer = document.WiseGridReer;
...
function doDelete(strColumnKey) {
	...
	var cnt = GridObjReer.GetRowCount()
		chkCount = 0;
	...
	for(var i=0; i<cnt; i++) {
		...
		if(GridObjReer.getCellValue("CRUD", i) != "NEW") {
			if(GridObjReer.GetCellValue("SELECTED", i) == "1")
				GridObjReer.DeleteRow(i);
		} else {
			if(GridObjReer.GetCellValue("SELECTED", i) == "1")
				GridObjReer.DeleteRow(i);
			i = i-1;
	...
	//GridObjReer.DeleteRow(GridObjReer.GetActiveRowIndex());
}
/*로우 체크 확인*/
function chkSelected() {
	chkCount = 0;

	for(i = 0; i < GridObjReer.GetRowCount(); i++) { //그리드 데이터의 로우수를 반환한다.

		if(GridObjReer.GetCellValue("SELECTED", i) == "1") //지정한 셀의 값을 가져온다.
			chkCount = chkCount + 1;
	}
...
function doSave() {
	...
	GridObjReer.SetParam("mode", "save_Reer");

	//WiseGrid가 서버와 통신시에 데이터를 전달한
	GridObjReer.DoQuery(servlet_url, "CRUD");
}
...
function GridEndQuery() {
	...
	var mode = GridObjReer.GetParam("mode");

		if(mode == "search") {
		if(GridObjReer.GetStatus() == "true") 	{// 서버에서 전송한 상태코드를 가져온다.
		} else	{
			var error_msg = GridObjReer.GetMessage(); // 서버에서 전송한 상태코드값이 false라면 에러메세지를 가져온다.
			alert(error_msg);
		}
	}
...
<table width="522" height="293" border="0" cellpadding="0" cellspacing="0">
  ...
  <tr>
    <td><script>initWiseGrid("WiseGridReer", "522", "400");</script></td>
  </tr>
</table>
</form>
</body>
